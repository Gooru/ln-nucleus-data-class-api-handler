...
public class IndependentLearnerUnitPerfHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(IndependentLearnerUnitPerfHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    for (String userID : userIds) {
      ...
      lessonIDforUnit = AJEntityBaseReports
          .findBySQL(AJEntityBaseReports.SELECT_INDEPENDENT_LEARNER_DISTINCT_LESSON_ID_FOR_UNIT_ID
              + addCollTypeFilterToQuery, context.courseId(), context.unitId(), userID);
      if (!lessonIDforUnit.isEmpty()) {
        ...
        List<Map> lessonKpi;
        if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
          lessonKpi = Base.findAll(
              AJEntityBaseReports.SELECT_INDEPENDENT_LEARNER_UNIT_PERF_FOR_COLLECTION,
              context.courseId(), context.unitId(), userID, listToPostgresArrayString(lessonIds));
        } else {
          lessonKpi =
              Base.findAll(AJEntityBaseReports.SELECT_INDEPENDENT_LEARNER_UNIT_PERF_FOR_ASSESSMENT,
          ...
          for (Map m : lessonKpi) {
            ...
            if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
              ...
              completedCountMap = Base.findAll(
                  AJEntityBaseReports.GET_COMPLETED_COLL_COUNT_FOREACH_INDEPENDENT_LEARNER_LESSON_ID,
              ...
              completedCountMap = Base.findAll(
                  AJEntityBaseReports.GET_COMPLETED_ASMT_COUNT_FOREACH_INDEPENDENT_LEARNER_LESSON_ID,
                  context.courseId(), context.unitId(), this.lessonId, userID,
                  EventConstants.COLLECTION_PLAY);
            }
            ...
            completedCountMap.forEach(scoreCompletonMap -> {
              ...
              lessonData
                  .put(AJEntityBaseReports.ATTR_SCORE,
            ...
            collIDforlesson = AJEntityBaseReports.findBySQL(
                AJEntityBaseReports.SELECT_INDEPENDENT_LEARNER_DISTINCT_COLLID_FOR_LESSON_ID_WO_PATH_ID
            ...
            if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
              ...
              assessmentKpi = Base.findAll(
                  AJEntityBaseReports.SELECT_INDEPENDENT_LEARNER_LESSON_PERF_FOR_COLLECTION_WO_PATH_ID,
                  context.courseId(), context.unitId(), this.lessonId,
                  listToPostgresArrayString(collIds), userID);
            } else {
              assessmentKpi = Base.findAll(
                  AJEntityBaseReports.SELECT_INDEPENDENT_LEARNER_LESSON_PERF_FOR_ASSESSMENT_WO_PATH_ID,
                  context.courseId(), context.unitId(), this.lessonId,
                ...
                if (this.collectionType.equalsIgnoreCase(JsonConstants.COLLECTION)) {
                  ...
                  collectionQuestionCount =
                      Base.findAll(AJEntityBaseReports.SELECT_IL_COLLECTION_SCORE_AND_MAX_SCORE,
                          context.courseId(), context.unitId(), this.lessonId,
                          assData.getString(AJEntityBaseReports.ATTR_ASSESSMENT_ID), userID);
                  ...
                  assData.put(AJEntityBaseReports.ATTR_COLLECTION_ID,
                      assData.getString(AJEntityBaseReports.ATTR_ASSESSMENT_ID));
