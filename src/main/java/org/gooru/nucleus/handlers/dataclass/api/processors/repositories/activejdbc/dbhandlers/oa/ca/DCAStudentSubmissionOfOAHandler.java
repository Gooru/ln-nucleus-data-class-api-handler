 ...
 * 
 */
public class DCAStudentSubmissionOfOAHandler implements DBHandler {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(DCAStudentSubmissionOfOAHandler.class);
  ...
  public DCAStudentSubmissionOfOAHandler(ProcessorContext context) {
  ...
  private void fetchStudentRubricData(JsonObject oaRubrics) {
    List<Model> ansModel = AJEntityOfflineActivitySelfGrade.where(
        AJEntityOfflineActivitySelfGrade.FETCH_CA_OA_SELF_GRADES, this.classId, this.oaDcaId,
        this.studentId);
  ...
  private JsonArray fetchTasks(JsonObject result) {
    ...
    List<Model> taskSubmissionsModel = AJEntityOfflineActivitySubmissions.where(
        AJEntityOfflineActivitySubmissions.FETCH_CA_OA_SUBMISSIONS, this.classId, this.oaDcaId,
        this.studentId);
