...
public class IndLearnerCourseCollectionsPerfHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(IndLearnerCourseCollectionsPerfHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    // NOTE: This code will need to be refactored going ahead. (based on changes/updates to Student
    // Performance Reports)
    ...
    if (StringUtil.isNullOrEmpty(userId)) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "User Id Missing. Cannot fetch Student Performance in Collection"),
          ExecutionStatus.FAILED);
    ...
    if (StringUtil.isNullOrEmpty(classId)) {
      query.append(AJEntityBaseReports.SPACE).append(AJEntityBaseReports.AND)
          .append(AJEntityBaseReports.SPACE).append("class_id IS NULL")
          .append(AJEntityBaseReports.SPACE);
    } else {
      query.append(AJEntityBaseReports.SPACE).append(AJEntityBaseReports.AND)
          .append(AJEntityBaseReports.SPACE).append(AJEntityBaseReports.CLASS_ID)
          .append(AJEntityBaseReports.SPACE);
      params.add(classId);
    }
    ...
    if (StringUtil.isNullOrEmpty(courseId)) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "Course Id Missing. Cannot fetch Student Performance in Collection"),
          ExecutionStatus.FAILED);

    } else {
      query.append(AJEntityBaseReports.AND).append(AJEntityBaseReports.SPACE)
          .append(AJEntityBaseReports.COURSE_ID_IS);
      params.add(courseId);
    }
    ...
    if (!StringUtil.isNullOrEmpty(unitId)) {
      query.append(AJEntityBaseReports.AND).append(AJEntityBaseReports.SPACE)
          .append(AJEntityBaseReports.UNIT_ID_IS);
      params.add(unitId);
    }
    ...
    if (!StringUtil.isNullOrEmpty(lessonId)) {
      query.append(AJEntityBaseReports.AND).append(AJEntityBaseReports.SPACE)
          .append(AJEntityBaseReports.LESSON_ID_IS);
      params.add(lessonId);
    }
    ...
    if (!StringUtil.isNullOrEmpty(startDate) && !FieldValidator.validateDate(startDate)) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "Invalid startDate. Cannot fetch Student Performance in Collection"),
          ExecutionStatus.FAILED);
    ...
    if (!StringUtil.isNullOrEmpty(startDate)) {
      query.append(AJEntityBaseReports.SPACE).append(AJEntityBaseReports.AND)
          .append(AJEntityBaseReports.SPACE)
          .append(AJEntityBaseReports.UPDATDED_AT_GREATER_THAN_OR_EQUAL);
    ...
    if (!StringUtil.isNullOrEmpty(endDate) && !FieldValidator.validateDate(endDate)) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "Invalid endDate. Cannot fetch Student Performance in Collection"),
          ExecutionStatus.FAILED);
    ...
    if (!StringUtil.isNullOrEmpty(endDate)) {
      query.append(AJEntityBaseReports.SPACE).append(AJEntityBaseReports.AND)
          .append(AJEntityBaseReports.SPACE)
          .append(AJEntityBaseReports.UPDATDED_AT_LESS_THAN_OR_EQUAL);
      params.add(endDate);
    }
    LOGGER.debug("Query : " + query);
    LazyList<AJEntityBaseReports> collectionList =
        AJEntityBaseReports.findBySQL(query.toString(), params.toArray());
    ...
    for (String collId : collIds) {
      ...
      StringBuilder collTSAQuery =
          new StringBuilder(AJEntityBaseReports.GET_IL_COURSE_COLLECTION_PERF);
      if (!StringUtil.isNullOrEmpty(startDate)) {
        collTSAQuery.append(AJEntityBaseReports.SPACE).append(AJEntityBaseReports.AND)
            .append(AJEntityBaseReports.SPACE)
            .append(AJEntityBaseReports.UPDATDED_AT_GREATER_THAN_OR_EQUAL);
      ...
      if (!collTSA.isEmpty()) {
        collTSA.forEach(m -> {
          collectionKpi.put(AJEntityBaseReports.ATTR_TIME_SPENT,
              Long.parseLong(m.get(AJEntityBaseReports.TIME_SPENT).toString()));
          collectionKpi.put(AJEntityBaseReports.VIEWS,
              Integer.parseInt(m.get(AJEntityBaseReports.VIEWS).toString()));
        });
      }
      List<Map> collectionMaximumScore;
      collectionMaximumScore =
          Base.findAll(AJEntityBaseReports.GET_IL_COLLECTION_MAX_SCORE, courseId, collId, userId);

      // If questions are not present then Question Count is always zero, however this additional
      ...
      if (this.maxScore > 0) {
        ...
        if (!StringUtil.isNullOrEmpty(classId)) {
          collectionScoreList = Base.findAll(AJEntityBaseReports.GET_COLLECTION_SCORE, classId,
              courseId, collId, userId);
        } else {
          collectionScoreList =
              Base.findAll(AJEntityBaseReports.GET_IL_COLLECTION_SCORE, courseId, collId, userId);
        }
        ...
        if (collectionScoreList != null && !collectionScoreList.isEmpty()) {
          ...
          if (collectionMap.get(AJEntityBaseReports.SESSION_ID) != null)
            latestSessionId = collectionMap.get(AJEntityBaseReports.SESSION_ID) != null
                ? collectionMap.get(AJEntityBaseReports.SESSION_ID).toString()
                : null;
        } else {
      ...
      // As per the current philosophy in Gooru (3.0/4.0) as soon as a collection is viewed, it is
      ...
      if (latestSessionId != null) {
        List<Map> inprogressListOfGradeStatus =
            Base.findAll(AJEntityBaseReports.FETCH_INPROGRESS_GRADE_STATUS_BY_SESSION_ID, userId,
                latestSessionId, collId);
