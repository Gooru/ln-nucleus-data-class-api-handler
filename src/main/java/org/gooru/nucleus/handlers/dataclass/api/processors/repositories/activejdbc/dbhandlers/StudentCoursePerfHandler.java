...
class StudentCoursePerfHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (context.classId() != null && StringUtil.isNullOrEmpty(userId)) {
      LOGGER.info(
          "UserID is not in the request fetch Student Performance in Course, Assume user is a teacher");
      LazyList<AJEntityBaseReports> userIDforCourse = AJEntityBaseReports.findBySQL(
          AJEntityBaseReports.SELECT_DISTINCT_USERID_FOR_COURSE_ID + addCollTypeFilterToQuery,
    ...
    for (String userID : userIds) {
      ...
      unitIDforCourse = AJEntityBaseReports.findBySQL(
          AJEntityBaseReports.SELECT_DISTINCT_UNIT_ID_FOR_COURSE_ID + addCollTypeFilterToQuery,
      ...
      if (!unitIDforCourse.isEmpty()) {
        ...
        if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
          assessmentKpi = Base.findAll(
              AJEntityBaseReports.SELECT_STUDENT_COURSE_PERF_FOR_COLLECTION, context.classId(),
              context.courseId(), userID, listToPostgresArrayString(unitIds));
        } else {
          assessmentKpi =
              Base.findAll(AJEntityBaseReports.SELECT_STUDENT_COURSE_PERF_FOR_ASSESSMENT,
                  context.classId(), context.courseId(), userID, listToPostgresArrayString(unitIds),
          ...
          assessmentKpi.forEach(m -> {
            ...
            if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
              completedCountMap = Base.findAll(
                  AJEntityBaseReports.GET_COMPLETED_COLL_COUNT_FOREACH_UNIT_ID, context.classId(),
                  context.courseId(), unitId, userID, EventConstants.COLLECTION_PLAY);
              scoreMap = Base.findAll(AJEntityBaseReports.GET_COLL_SCORE_FOREACH_UNIT_ID,
                  context.classId(), context.courseId(), unitId, userID);
            } else {
              completedCountMap = Base.findAll(
                  AJEntityBaseReports.GET_COMPLETED_ASS_COUNT_FOREACH_UNIT_ID, context.classId(),
                  context.courseId(), unitId, userID, EventConstants.COLLECTION_PLAY);
            }
            JsonObject unitData =
