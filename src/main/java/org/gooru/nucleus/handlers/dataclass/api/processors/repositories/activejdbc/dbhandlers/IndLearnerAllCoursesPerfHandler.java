...
public class IndLearnerAllCoursesPerfHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(IndLearnerAllCoursesPerfHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (StringUtil.isNullOrEmpty(this.userId)) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "UserId Missing. Cannot fetch IL performance in all courses"),
    ...
    if (courseIds.isEmpty()) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "CourseIds is Missing. Cannot fetch IL Performance in Course"),
          ExecutionStatus.FAILED);
    }
    ...
    coursePerfData = Base.findAll(AJEntityBaseReports.SELECT_IL_ALL_COURSE_DATA,
        jArrayToPostgresArrayString(courseIds), this.userId);
    ...
    if (!coursePerfData.isEmpty()) {
      coursePerfData.forEach(courseData -> {
        if (courseData.get(AJEntityBaseReports.COURSE_GOORU_OID) != null) {
          ...
          courseKPI.put(AJEntityBaseReports.ATTR_TIME_SPENT,
              Long.valueOf(courseData.get(AJEntityBaseReports.ATTR_TIME_SPENT).toString()));
          Object courseTotalCount =
          ...
          courseKPI.put(AJEntityBaseReports.ATTR_TOTAL_COUNT,
              courseTotalCount != null ? Integer.valueOf(courseTotalCount.toString()) : 0);
          ...
          if (!cScoreCompletion.isEmpty()) {
            cScoreCompletion.forEach(scoreKPI -> {
              LOGGER.debug("completedCount : {} ",
                  scoreKPI.get(AJEntityBaseReports.ATTR_COMPLETED_COUNT));
              LOGGER.debug("score : {} ", scoreKPI.get(AJEntityBaseReports.ATTR_SCORE));
              courseKPI.put(AJEntityBaseReports.ATTR_COMPLETED_COUNT, Integer
                  .valueOf(scoreKPI.get(AJEntityBaseReports.ATTR_COMPLETED_COUNT).toString()));
              courseKPI.put(AJEntityBaseReports.ATTR_SCORE,
                  scoreKPI.get(AJEntityBaseReports.ATTR_SCORE) == null ? null
                      : Math.round(
                          Double.valueOf(scoreKPI.get(AJEntityBaseReports.ATTR_SCORE).toString())));
            });
          } else {
            courseKPI.put(AJEntityBaseReports.ATTR_COMPLETED_COUNT, 0);
            courseKPI.putNull(AJEntityBaseReports.ATTR_SCORE);
          }
          courseKPI.put(AJEntityBaseReports.ATTR_COURSE_ID,
              courseData.get(AJEntityBaseReports.COURSE_GOORU_OID).toString());
