...
public class DCASessionTaxonomyReportHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(DCASessionTaxonomyReportHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    List<Map> sessionTaxonomyResults =
        Base.findAll(AJEntityDCACompetencyReport.SELECT_DCA_REPORT_IDS, this.context.sessionId());
    if (!sessionTaxonomyResults.isEmpty()) {
      sessionTaxonomyResults.forEach(taxonomyRow -> {
        ...
        aggResult.put(JsonConstants.DISPLAY_CODE,
            taxonomyRow.get(AJEntityDCACompetencyReport.DISPLAY_CODE));

        if (taxonomyRow.get(AJEntityDCACompetencyReport.TAX_MICRO_STANDARD_ID) != null) {
          aggResult.put(JsonConstants.LEARNING_TARGET_ID,
              taxonomyRow.get(AJEntityDCACompetencyReport.TAX_MICRO_STANDARD_ID));
        } else {
          aggResult.put(JsonConstants.STANDARDS_ID,
              taxonomyRow.get(AJEntityDCACompetencyReport.TAX_STANDARD_ID));
        }

        LOGGER.debug("Base Reports IDS : {}", listToPostgresArrayInteger(
            taxonomyRow.get(AJEntityDCACompetencyReport.BASE_REPORT_ID).toString()));
        List<Map> aggTaxonomyResults = Base
            .findAll(AJEntityDCACompetencyReport.GET_DCA_AGG_TAX_DATA, listToPostgresArrayInteger(
                taxonomyRow.get(AJEntityDCACompetencyReport.BASE_REPORT_ID).toString()));

        if (!aggTaxonomyResults.isEmpty()) {
          aggTaxonomyResults.forEach(aggData -> {
            aggResult.put(JsonConstants.TIMESPENT,
                Long.valueOf(aggData.get(AJEntityDailyClassActivity.TIMESPENT).toString()));
            aggResult.put(JsonConstants.REACTION,
                Integer.valueOf(aggData.get(AJEntityDailyClassActivity.REACTION).toString()));
            aggResult.put(JsonConstants.SCORE, Math
                .round(Double.valueOf(aggData.get(AJEntityDailyClassActivity.SCORE).toString())));

          });
        }
        List<Map> sessionTaxonomyQuestionResults = Base.findAll(
            AJEntityDCACompetencyReport.GET_DCA_QUESTIONS_TAX_PERF, listToPostgresArrayInteger(
                taxonomyRow.get(AJEntityDCACompetencyReport.BASE_REPORT_ID).toString()));
        ...
        if (!sessionTaxonomyQuestionResults.isEmpty()) {
          ...
          sessionTaxonomyQuestionResults.forEach(question -> {
            ...
            questionData.put(JsonConstants.SCORE, Math
                .round(Double.valueOf(question.get(AJEntityDailyClassActivity.SCORE).toString())));
