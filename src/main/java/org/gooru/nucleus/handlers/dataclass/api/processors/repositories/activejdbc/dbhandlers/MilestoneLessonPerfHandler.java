...
public class MilestoneLessonPerfHandler implements DBHandler {
    ...
    public ExecutionResult<MessageResponse> checkSanity() {
      if (context.request() == null || context.request().isEmpty()) {
        ...
        return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse("Invalid data provided to fetch Student Performance in Milestones"),
    ...
    public ExecutionResult<MessageResponse> executeRequest() {
      ...
      if (StringUtil.isNullOrEmpty(userId)) {
      	LOGGER.warn("UserID is not in the request to fetch Student Performance in Milestones. Assume user is a teacher");
      	LazyList<AJEntityBaseReports> userIdforMilestones =
      			AJEntityBaseReports.findBySQL(AJEntityBaseReports.SELECT_DISTINCT_USERID_FOR_MILESTONE + addCollTypeFilterToQuery, context.classId(),
      					context.courseId(), listToPostgresArrayString(lessonIds));
      	userIdforMilestones.forEach(lesson -> userIds.add(lesson.getString(AJEntityBaseReports.GOORUUID)));
      } else {        
      	userIds.add(userId);
      }
      ...
      for (String userID : userIds) {
        ...
        if (!lessonIDforMilestone.isEmpty()) {                    
          ...
          if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
            lessonDataList = Base.findAll(AJEntityBaseReports.SELECT_STUDENT_MILESTONE_LESSON_PERF_FOR_COLLECTION, context.classId(), context.courseId(),
                  userID, listToPostgresArrayString(lessonIds));
          } else {
        	  lessonDataList = Base.findAll(AJEntityBaseReports.SELECT_STUDENT_MILESTONE_LESSON_PERF_FOR_ASSESSMENT, context.classId(), context.courseId(),
          ...
          if (!lessonDataList.isEmpty()) {
            for (Map m : lessonDataList) {
              ...
              if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
            	  scoreCompletionMap = Base.findAll(AJEntityBaseReports.GET_COLL_COMPLETED_COUNT_SCORE_FOREACH_LESSON_ID, context.classId(),
                      context.courseId(), this.lessonId, userID, EventConstants.COLLECTION_PLAY);
                scoreMap = Base.findAll(AJEntityBaseReports.GET_MILESTONE_COLL_SCORE_FOREACH_LESSON_ID, context.classId(),
                        context.courseId(), this.lessonId, userID);
              }else{
            	  scoreCompletionMap = Base.findAll(AJEntityBaseReports.GET_ASMT_COMPLETED_COUNT_SCORE_FOREACH_LESSON_ID, context.classId(),
                        context.courseId(), this.lessonId, userID, EventConstants.COLLECTION_PLAY, EventConstants.STOP);
              }
              JsonObject lessonData = ValueMapper.map(ResponseAttributeIdentifier.getMilestoneLessonPerformanceAttributesMap(), m);
              scoreCompletionMap.forEach(sc -> {
                lessonData.put(AJEntityBaseReports.ATTR_COMPLETED_COUNT, Integer.valueOf(sc.get(AJEntityBaseReports.ATTR_COMPLETED_COUNT).toString()));
                lessonData.put(AJEntityBaseReports.ATTR_SCORE,  sc.get(AJEntityBaseReports.ATTR_SCORE) != null ?
              		  Math.round(Double.valueOf(sc.get(AJEntityBaseReports.ATTR_SCORE).toString())) : null);
                lessonData.put(AJEntityBaseReports.ATTR_UNIT_ID, sc.get(AJEntityBaseReports.ATTR_UNIT_ID) != null ?
                		  sc.get(AJEntityBaseReports.ATTR_UNIT_ID).toString() : null);
              });
              ...
              if (lessonAssessmentCountMap.containsKey(this.lessonId)) {
              	...
              	Object classTotalCount = Base.firstCell(AJEntityCourseCollectionCount.GET_MILESTONE_LESSON_ASSESSMENT_COUNT,
              ...
              if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
                ...
                if (lessonCollectionCountMap.containsKey(this.lessonId)) {
                  	...
                  	Object classTotalCount = Base.firstCell(AJEntityCourseCollectionCount.GET_MILESTONE_LESSON_COLLECTION_COUNT,
                  			context.courseId(), this.lessonId);
                  	totalCount = classTotalCount != null ? (Integer.valueOf(classTotalCount.toString())) : 0;
        ...
        contentBody.put(JsonConstants.USAGE_DATA, MilestoneDataArray).put(JsonConstants.USERUID, userID);
