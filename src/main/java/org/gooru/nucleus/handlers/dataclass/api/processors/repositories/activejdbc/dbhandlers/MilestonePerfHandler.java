...
public class MilestonePerfHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (context.classId() != null && StringUtil.isNullOrEmpty(userId)) {
      LOGGER.info(
          "UserID is not in the request fetch Student Performance in Milestone, Assume user is a teacher");
    ...
    for (String mileId : milestoneIds) {
      ...
      if (!lessonIDforMilestone.isEmpty()) {
        for (String userID : userIds) {
          ...
          if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
            assessmentPerf = Base.findAll(
                AJEntityBaseReports.SELECT_STUDENT_MILESTONE_PERF_FOR_COLLECTION, context.classId(),
                context.courseId(), userID, listToPostgresArrayString(lessonIds));
          } else {
            assessmentPerf =
                Base.findAll(AJEntityBaseReports.SELECT_STUDENT_MILESTONE_PERF_FOR_ASSESSMENT,
            ...
            assessmentPerf.forEach(m -> {
              ...
              if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
                scoreCompletionMap = Base.findAll(
                    AJEntityBaseReports.GET_COMPLETED_COLL_COMP_COUNT_SCORE_FOREACH_MILESTONE_ID,
                    context.classId(), context.courseId(), listToPostgresArrayString(lessonIds),
                    userID, EventConstants.COLLECTION_PLAY);
                scoreMap = Base.findAll(AJEntityBaseReports.GET_COLL_SCORE_FOREACH_MILESTONE_ID,
                    context.classId(), context.courseId(), listToPostgresArrayString(lessonIds),
                    userID);
              } else {
                scoreCompletionMap = Base.findAll(
                    AJEntityBaseReports.GET_COMPLETED_ASMT_COMP_COUNT_SCORE_FOREACH_MILESTONE_ID,
                    context.classId(), context.courseId(), listToPostgresArrayString(lessonIds),
                    userID, EventConstants.COLLECTION_PLAY);
              }
