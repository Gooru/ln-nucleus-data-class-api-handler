...
public class StudDCACollectionPerfHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (StringUtil.isNullOrEmpty(tDate)) {
      LOGGER.warn(
          "Date is mandatory to fetch Student Collection Performance in Daily Class Activity.");
      return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
          "Date is Missing. Cannot fetch Student Collection Performance in Daily Class Activity"),
          ExecutionStatus.FAILED);
    ...
    if (context.classId() != null && StringUtil.isNullOrEmpty(userId)) {
      ...
      LazyList<AJEntityDailyClassActivity> userIdforCollection =
          AJEntityDailyClassActivity.findBySQL(
              AJEntityDailyClassActivity.SELECT_DISTINCT_USERID_FOR_COLLECTION_ID
                  + AJEntityDailyClassActivity.COLL_TYPE_FILTER,
        ...
        if (!collectionQuestionsKPI.isEmpty()) {
          collectionQuestionsKPI.forEach(questions -> {
            ...
            if (questions.get(AJEntityDailyClassActivity.ANSWER_OBJECT) != null) {
              qnData.put(JsonConstants.ANSWER_OBJECT, new JsonArray(
            ...
            List<Map> questionScore = Base.findAll(
                AJEntityDailyClassActivity.SELECT_COLLECTION_QUESTION_AGG_SCORE, classId,
            ...
            if (questionScore != null && !questionScore.isEmpty()) {
              latestSessionId =
                  (questionScore.get(0).get(AJEntityDailyClassActivity.SESSION_ID) != null)
                      ? questionScore.get(0).get(AJEntityDailyClassActivity.SESSION_ID).toString()
                      : null;
              questionScore.forEach(qs -> {
                ...
                qnData.put(JsonConstants.RAW_SCORE,
                    qs.get(AJEntityDailyClassActivity.SCORE) != null
                        ? Math.round(
                            Double.valueOf(qs.get(AJEntityDailyClassActivity.SCORE).toString()))
                        : "NA");
                ...
                qnData.put(JsonConstants.MAX_SCORE,
                    qs.get(AJEntityDailyClassActivity.MAX_SCORE) != null
                        ? Double.valueOf(qs.get(AJEntityDailyClassActivity.MAX_SCORE).toString())
                        : "NA");
                Double quesScore = qs.get(AJEntityDailyClassActivity.SCORE) != null
                    ? Double.valueOf(qs.get(AJEntityDailyClassActivity.SCORE).toString())
                    : null;
                Double maxScore = qs.get(AJEntityDailyClassActivity.MAX_SCORE) != null
                    ? Double.valueOf(qs.get(AJEntityDailyClassActivity.MAX_SCORE).toString())
                    : 0.0;
                double scoreInPercent;
            ...
            if (latestSessionId != null && qnData.getString(EventConstants.QUESTION_TYPE)
              ...
              Object isGradedObj =
                  Base.firstCell(AJEntityDailyClassActivity.GET_OE_QUE_GRADE_STATUS, collectionId,
                      latestSessionId, questions.get(AJEntityDailyClassActivity.RESOURCE_ID), date);
            ...
            resourceReaction = Base.findAll(
                AJEntityDailyClassActivity.SELECT_COLLECTION_RESOURCE_AGG_REACTION, classId,
                collectionId, questions.get(AJEntityDailyClassActivity.RESOURCE_ID), userID, date);
            ...
            if (!resourceReaction.isEmpty()) {
              resourceReaction.forEach(rs -> {
                ...
                LOGGER.debug("Resource reaction: {} - resourceId : {}",
                    rs.get(AJEntityDailyClassActivity.REACTION).toString(),
