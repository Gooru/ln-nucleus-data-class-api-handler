...
public class UserCollectionSessionsHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    LOGGER.debug("classId :{} , userId : {} and collectionId:{}", classId, userId,
        this.collectionId);
    ...
    if (StringUtil.isNullOrEmpty(userId)) {
      ...
      return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
          "UserID Missing. Cannot fetch Session Information"), ExecutionStatus.FAILED);
    }
    LOGGER.debug("UID is " + userId);
    List<Map> distinctSessionsList;
    if (!StringUtil.isNullOrEmpty(classId)) {
      distinctSessionsList =
          Base.findAll(AJEntityBaseReports.GET_USER_SESSIONS_FOR_COLLID, classId, courseId,


              unitId,


              lessonId, this.collectionId, EventConstants.ASSESSMENT, userId);
    } else {
    ...
    if (!distinctSessionsList.isEmpty()) {
      ...
      distinctSessionsList.forEach(m -> {
        ...
        List<Map> sessionStatusMap = Base.findAll(AJEntityBaseReports.GET_SESSION_STATUS, sessionId,
            this.collectionId, EventConstants.COLLECTION_PLAY);

        // TODO: Covert TimeStamp, also ORDER BY DESC so that the higher sequence number will mean
        // latest sessions
        if (!sessionStatusMap.isEmpty()) {

          sessionStatusMap.forEach(sess -> {
            if (Objects.equals(sess.get(EventConstants.EVENT_TYPE).toString(),
                EventConstants.START)) {
              sessionObj
                  .put(JsonConstants.EVENT_TIME,
                      sess.get(AJEntityBaseReports.CREATE_TIMESTAMP).toString())
    ...
    if (openSession.equalsIgnoreCase("false")) {
      resultBody.put(JsonConstants.CONTENT, closedSessionArray).putNull(JsonConstants.MESSAGE)
          .putNull(JsonConstants.PAGINATE);
    } else if (openSession.equalsIgnoreCase("true")) {
      resultBody.put(JsonConstants.CONTENT, openSessionArray).putNull(JsonConstants.MESSAGE)
          .putNull(JsonConstants.PAGINATE);
    }
