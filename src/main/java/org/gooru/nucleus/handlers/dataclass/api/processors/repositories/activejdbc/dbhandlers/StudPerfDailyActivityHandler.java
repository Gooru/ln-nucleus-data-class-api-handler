...
public class StudPerfDailyActivityHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    for (String userId : userIds) {
      ...
      if (collectionType.equalsIgnoreCase(JsonConstants.ASSESSMENT)) {
        ...
        activityList =
            Base.findAll(AJEntityDailyClassActivity.GET_PERFORMANCE_FOR_CLASS_ASSESSMENTS, classId,
                PgUtils.listToPostgresArrayString(collIds), userId,
                AJEntityDailyClassActivity.ATTR_CP_EVENTNAME, startDate, endDate);
      } else {
        LOGGER.debug("Fetching Performance for Collections in Class");
        activityList =
            Base.findAll(AJEntityDailyClassActivity.GET_PERFORMANCE_FOR_CLASS_COLLECTIONS, classId,
  ...
  private void generateActivityData(JsonArray activityArray, List<Map> activityList) {
    activityList.forEach(m -> {
      ...
      contentKpi.put(AJEntityDailyClassActivity.DATE,
  ...
  private void calculateCollectionScore(String userId, Map m, JsonObject collectionKpi) {
    ...
    Object collectionScore =
        Base.firstCell(AJEntityDailyClassActivity.GET_PERFORMANCE_FOR_CLASS_COLLECTIONS_SCORE,
  ...
  private void validateContextRequestFields() {
    if (StringUtil.isNullOrEmpty(eDate) || StringUtil.isNullOrEmpty(sDate)) {
      LOGGER.warn(
          "Start Date and End Date are mandatory to fetch Student Performance in Daily Class Activity.");
    ...
    if (StringUtil.isNullOrEmpty(classId)) {
      ...
      throw new MessageResponseWrapperException(MessageResponseFactory.createNotFoundResponse(
          "Class Id Missing. Cannot fetch Student Performance in Daily Class Activity"));
    }
  }
}
