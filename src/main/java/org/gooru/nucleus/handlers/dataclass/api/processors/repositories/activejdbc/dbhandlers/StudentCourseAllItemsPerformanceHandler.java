...
public class StudentCourseAllItemsPerformanceHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(StudentCourseAllItemsPerformanceHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    for (String userID : userIds) {
      ...
      List<Map> assessmentKpi =
          Base.findAll(AJEntityCollectionPerformance.SELECT_ITEM_PERF_IN_CLASS, context.classId(),
              courseId, userID, startDate, offset, limit);
      if (!assessmentKpi.isEmpty()) {
        ...
        assessmentKpi.forEach(m -> {
          ...
          Double score = m.get(AJEntityBaseReports.SCORE) != null
              ? Double.valueOf(m.get(AJEntityBaseReports.SCORE).toString())
              : null;
          Double maxScore = m.get(AJEntityBaseReports.ATTR_MAX_SCORE) != null
              ? Double.valueOf(m.get(AJEntityBaseReports.ATTR_MAX_SCORE).toString())
              : null;
          if (collectionType.equalsIgnoreCase(JsonConstants.COLLECTION)) {
            ...
            collectionKpi.put(EventConstants.VIEWS,
                (m.get(AJEntityBaseReports.ATTR_ATTEMPTS) != null
                    && Integer.valueOf(m.get(AJEntityBaseReports.ATTR_ATTEMPTS).toString()) > 0)
                        ? Integer.valueOf(m.get(AJEntityBaseReports.ATTR_ATTEMPTS).toString())
                        : 1);
          } else {
          ...
          collectionKpi.put(AJEntityBaseReports.ATTR_SESSION_ID,
              m.get(AJEntityBaseReports.ATTR_SESSION_ID).toString());
          ...
          collectionKpi.put(AJEntityBaseReports.ATTR_UNIT_ID,
              m.get(AJEntityBaseReports.ATTR_UNIT_ID) != null
                  ? m.get(AJEntityBaseReports.ATTR_UNIT_ID).toString()
                  : null);
          collectionKpi.put(AJEntityBaseReports.ATTR_LESSON_ID,
              m.get(AJEntityBaseReports.ATTR_LESSON_ID) != null
                  ? m.get(AJEntityBaseReports.ATTR_LESSON_ID).toString()
                  : null);
          collectionKpi.put(AJEntityBaseReports.ATTR_PATH_ID,
              (m.get(AJEntityBaseReports.ATTR_PATH_ID) != null
                  && Integer.valueOf(m.get(AJEntityBaseReports.ATTR_PATH_ID).toString()) > 0)
                      ? Integer.valueOf(m.get(AJEntityBaseReports.ATTR_PATH_ID).toString())
                      : null);
          collectionKpi.put(AJEntityBaseReports.ATTR_PATH_TYPE,
              m.get(AJEntityBaseReports.ATTR_PATH_TYPE) != null
                  ? m.get(AJEntityBaseReports.ATTR_PATH_TYPE).toString()
                  : null);
          collectionKpi.put(AJEntityBaseReports.ATTR_LAST_ACCESSED,
              m.get(AJEntityBaseReports.UPDATE_TIMESTAMP).toString());
          collectionKpi.put(AJEntityBaseReports.ATTR_CONTENT_SOURCE,
              m.get(AJEntityBaseReports.ATTR_CONTENT_SOURCE) != null
                  ? m.get(AJEntityBaseReports.ATTR_CONTENT_SOURCE).toString()
                  : null);

          collectionKpi.put(AJEntityBaseReports.ATTR_TIME_SPENT,
              m.get(AJEntityBaseReports.ATTR_TIME_SPENT) != null
                  ? Long.valueOf(m.get(AJEntityBaseReports.ATTR_TIME_SPENT).toString())
                  : 0);
          collectionKpi.put(AJEntityBaseReports.ATTR_REACTION,
              m.get(AJEntityBaseReports.ATTR_REACTION) != null
                  ? Integer.valueOf(m.get(AJEntityBaseReports.ATTR_REACTION).toString())
                  : 0);

          collectionKpi.put(JsonConstants.STATUS,
        ...
        Object studClassStartDate =
            Base.firstCell(AJEntityCollectionPerformance.GET_STUDENT_CLASS_ACTIVITY_START_DATE,
                context.classId(), courseId, userID);
        String studentClassStartDate =
            studClassStartDate != null ? studClassStartDate.toString() : null;
        contentBody.put(JsonConstants.USAGE_DATA, collectionKpiArray)
            .put(JsonConstants.USERUID, userID)
            .put(JsonConstants.START_DATE, studentClassStartDate);
  ...
  private void validateContextRequest() {
    if (context.request() == null || context.request().isEmpty()) {
      ...
      throw new MessageResponseWrapperException(MessageResponseFactory
          .createInvalidRequestResponse("Invalid data provided to fetch items"));
    }
  }
  ...
  private void validateContextRequestFields() {
    if (StringUtil.isNullOrEmpty(sDate)) {
      ...
      throw new MessageResponseWrapperException(MessageResponseFactory
          .createNotFoundResponse("Start Date is Missing. Cannot fetch items"));
    }
    if (StringUtil.isNullOrEmpty(classId)) {
      LOGGER.warn("ClassId is mandatory for fetching items");
      throw new MessageResponseWrapperException(
          MessageResponseFactory.createNotFoundResponse("Class Id Missing. Cannot fetch items"));
    }
    if (StringUtil.isNullOrEmpty(courseId)) {
      LOGGER.warn("CourseId is mandatory for fetching items");
      throw new MessageResponseWrapperException(
          MessageResponseFactory.createNotFoundResponse("Course Id Missing. Cannot fetch items"));
    }
    if (limit < 0 || offset < 0) {
      LOGGER.warn("Limit/Offset requested is negative");
      throw new MessageResponseWrapperException(MessageResponseFactory
          .createNotFoundResponse("Limit/Offset requested should not be negative"));
    }
  }
  ...
  private List<String> fetchUserId() {
    ...
    if (StringUtil.isNullOrEmpty(userId)) {
      LOGGER.warn(
          "UserID is not in the request to fetch Student All Items Perf in Class. Assume user is a teacher");
      LazyList<AJEntityCollectionPerformance> userIdOfClass = AJEntityCollectionPerformance
          .findBySQL(AJEntityCollectionPerformance.SELECT_DISTINCT_USERID_FOR_COURSE_ID,
