...
public class IndLearnerAssessmentSummaryHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(IndLearnerAssessmentSummaryHandler.class);
    ...
    if (context.getUserIdFromRequest() == null) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createForbiddenResponse(
              "UserID is mandatory to fetch Independent Learner Assessment Performance"),
          ExecutionStatus.FAILED);
    }
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (!StringUtil.isNullOrEmpty(sessionId)) {
      List<Map> assessmentKPI =
          Base.findAll(AJEntityBaseReports.SELECT_IL_ASSESSMENT_FOREACH_COLLID_AND_SESSION_ID,
              context.collectionId(), sessionId, AJEntityBaseReports.ATTR_CP_EVENTNAME);
      Object assessmentReactionObject =
          Base.firstCell(AJEntityBaseReports.SELECT_IL_ASSESSMENT_REACTION_AND_SESSION_ID,
      ...
      if (!assessmentKPI.isEmpty()) {
        ...
        assessmentKPI.forEach(m -> {
          JsonObject assessmentData =
              ValueMapper.map(ResponseAttributeIdentifier.getSessionAssessmentAttributesMap(), m);
        ...
        List<Map> assessmentQuestionsKPI = Base.findAll(
            AJEntityBaseReports.SELECT_IL_ASSESSMENT_QUESTION_FOREACH_COLLID_AND_SESSION_ID,
            context.collectionId(), sessionId, AJEntityBaseReports.ATTR_CRP_EVENTNAME);
        ...
        if (!assessmentQuestionsKPI.isEmpty()) {
          assessmentQuestionsKPI.forEach(questions -> {
            ...
            Object reactionObj = Base.firstCell(
                AJEntityBaseReports.SELECT_IL_ASSESSMENT_RESOURCE_REACTION, context.collectionId(),
        ...
        LOGGER.info("IL Assessment Attributes cannot be obtained");
      }
      resultBody.put(JsonConstants.CONTENT, contentArray);
      return new ExecutionResult<>(MessageResponseFactory.createGetResponse(resultBody),
          ExecutionStatus.SUCCESSFUL);
    } else {
      LOGGER.info("SessionID Missing, Cannot Obtain IL Assessment Perf data");
