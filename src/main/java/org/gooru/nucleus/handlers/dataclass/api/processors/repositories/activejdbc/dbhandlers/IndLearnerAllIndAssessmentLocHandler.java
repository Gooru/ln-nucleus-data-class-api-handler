...
public class IndLearnerAllIndAssessmentLocHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(IndLearnerAllIndAssessmentLocHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (StringUtil.isNullOrEmpty(userId)) {
      ...
      return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
          "User Id Missing. Cannot fetch Learner's Assessments"), ExecutionStatus.FAILED);
    ...
    ILAssessments =
        Base.findAll(AJEntityBaseReports.GET_DISTINCT_ASSESSMENT_FOR_INDEPENDENT_LEARNER, userId);
    ...
    if (!ILAssessments.isEmpty()) {
      ILAssessments.forEach(m -> {
        ...
        List<Map> completedAssessment = Base.findAll(AJEntityBaseReports.GET_IL_COLLECTION_STATUS,
        ...
        if (!completedAssessment.isEmpty()) {
          ...
          ILloc.put(AJEntityBaseReports.ATTR_SCORE,
              score.get(AJEntityBaseReports.ATTR_SCORE) == null ? null
                  : Math
                      .round(Double.valueOf(score.get(AJEntityBaseReports.ATTR_SCORE).toString())));
        } else {
