...
public class StudentLessonPerfHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (StringUtil.isNullOrEmpty(collectionType)) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "CollectionType Missing. Cannot fetch Student Performance in Lesson"),
          ExecutionStatus.FAILED);
    }
    ...
    if (context.classId() != null && StringUtil.isNullOrEmpty(userId)) {
      ...
      LazyList<AJEntityBaseReports> userIdforlesson = AJEntityBaseReports.findBySQL(
          AJEntityBaseReports.SELECT_DISTINCT_USERID_FOR_LESSON_ID + addCollTypeFilterToQuery,
    ...
    for (String userID : userIds) {
      ...
      collIDforlesson = AJEntityBaseReports.findBySQL(
          AJEntityBaseReports.SELECT_DISTINCT_COLLID_FOR_LESSON_ID + addCollTypeFilterToQuery,
          context.classId(), context.courseId(), context.unitId(), context.lessonId(), userID);
      ...
      if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
        assessmentKpi = Base.findAll(AJEntityBaseReports.SELECT_STUDENT_LESSON_PERF_FOR_COLLECTION,
            context.classId(), context.courseId(), context.unitId(), context.lessonId(),
            listToPostgresArrayString(collIds), userID);
      } else {
        assessmentKpi = Base.findAll(AJEntityBaseReports.SELECT_STUDENT_LESSON_PERF_FOR_ASSESSMENT,
            context.classId(), context.courseId(), context.unitId(), context.lessonId(),
      ...
      if (!assessmentKpi.isEmpty()) {
        assessmentKpi.forEach(m -> {
          ...
          // In Gooru 3.0, total_count was hardcoded to 1 at this last mile, assessment/collection
          // level
          ...
          if (this.collectionType.equalsIgnoreCase(JsonConstants.COLLECTION)) {
            ...
            if (!isTeacher) {
              ...
              resourceKpi = Base.findAll(AJEntityBaseReports.GET_RESOURCE_PERF, context.classId(),
                  context.courseId(), context.unitId(), context.lessonId(), cId, userID,
              ...
              if (!resourceKpi.isEmpty()) {
                resourceKpi.forEach(res -> {
                  ...
                  resKpi.put(AJEntityBaseReports.ATTR_PATH_ID,
                      res.get(AJEntityBaseReports.ATTR_PATH_ID) == null ? 0L
                          : Long.parseLong(res.get(AJEntityBaseReports.ATTR_PATH_ID).toString()));
                  resKpi.put(AJEntityBaseReports.ATTR_RESOURCE_ID,
                      res.get(AJEntityBaseReports.ATTR_RESOURCE_ID).toString());
                  resKpi.put(AJEntityBaseReports.ATTR_TIME_SPENT,
                      Long.parseLong(res.get(AJEntityBaseReports.ATTR_TIME_SPENT).toString()));
                  resKpi.put(AJEntityBaseReports.VIEWS,
                      Integer.parseInt(res.get(AJEntityBaseReports.VIEWS).toString()));
      ...
      contentBody.put(JsonConstants.USAGE_DATA, LessonKpiArray)
          .put(JsonConstants.ALTERNATE_PATH, altPathArray).put(JsonConstants.USERUID, userID);
