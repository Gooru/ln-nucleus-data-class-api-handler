...
public class IndLearnerAllIndCollectionPerfHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(IndLearnerAllIndCollectionPerfHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    String query =
        StringUtil.isNullOrEmpty(limitS) ? AJEntityBaseReports.GET_IL_ALL_COLLECTION_VIEWS_TIMESPENT
            : AJEntityBaseReports.GET_IL_ALL_COLLECTION_VIEWS_TIMESPENT + "LIMIT "
                + Long.valueOf(limitS);

    List<Map> collectionAggData =
        Base.findAll(query, userId, StringUtil.isNullOrEmpty(offsetS) ? 0L : Long.valueOf(offsetS));
    if (!collectionAggData.isEmpty()) {
      collectionAggData.forEach(collectionsKpi -> {
        JsonObject collectionObj = new JsonObject();
        collectionObj.put(AJEntityBaseReports.ATTR_COLLECTION_ID,
