...
public class AJEntityDCACompetencyReport extends Model {
  ...
  public static final String GET_DCA_AGG_TAX_DATA =
      "SELECT sum(time_spent) AS time_spent, (AVG(score * 100)) AS score,"
          + "ROUND(AVG(reaction)) AS reaction "
          + "FROM daily_class_activity WHERE id = ANY (?::integer[]) GROUP BY session_id";
