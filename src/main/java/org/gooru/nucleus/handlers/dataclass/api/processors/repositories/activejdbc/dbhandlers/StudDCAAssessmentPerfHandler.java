...
public class StudDCAAssessmentPerfHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (StringUtil.isNullOrEmpty(tDate)) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "Date is Missing. Cannot fetch Student Performance in Daily Class Activity"),
          ExecutionStatus.FAILED);
    ...
    if (this.context.classId() != null && StringUtil.isNullOrEmpty(userId)) {
      LOGGER.warn(
          "UserID is not in the request to fetch Student Performance in DCA Collection. Assume user is a teacher");
      LazyList<AJEntityDailyClassActivity> userIdforlesson = AJEntityDailyClassActivity.findBySQL(
          AJEntityDailyClassActivity.SELECT_DISTINCT_USERID_FOR_ASSESSMENT
              + AJEntityDailyClassActivity.ASMT_TYPE_FILTER,
          context.classId(), context.collectionId(), date,
          AJEntityDailyClassActivity.ATTR_CP_EVENTNAME,
          AJEntityDailyClassActivity.ATTR_EVENTTYPE_STOP);
      userIdforlesson
          .forEach(coll -> userIds.add(coll.getString(AJEntityDailyClassActivity.GOORUUID)));
    } else {
      userIds.add(userId);
    }
    ...
    for (String userID : userIds) {
      ...
      AJEntityDailyClassActivity dcaAssessmentPerfModel = AJEntityDailyClassActivity.findFirst(
          "actor_id = ? AND class_id = ? AND collection_id = ? "
      ...
      if (dcaAssessmentPerfModel != null) {
        String studentLatestSessionId =
            dcaAssessmentPerfModel.getString(AJEntityDailyClassActivity.SESSION_ID);
        Object assessmentReactionObject =
            Base.firstCell(AJEntityDailyClassActivity.SELECT_ASSESSMENT_REACTION,
                context.collectionId(), studentLatestSessionId, date);
        ...
        assessmentData.put(JsonConstants.SCORE,
            dcaAssessmentPerfModel.get(AJEntityDailyClassActivity.SCORE) != null
                ? Math.round(Double.valueOf(
                    dcaAssessmentPerfModel.get(AJEntityDailyClassActivity.SCORE).toString()))
                : null);
        ...
        assessmentData.put(JsonConstants.TIMESPENT,
            dcaAssessmentPerfModel.get(AJEntityDailyClassActivity.TIMESPENT) != null
                ? dcaAssessmentPerfModel.getLong(AJEntityDailyClassActivity.TIMESPENT)
                : 0);
        assessmentData.put(JsonConstants.COLLECTION_TYPE,
            dcaAssessmentPerfModel.get(AJEntityDailyClassActivity.COLLECTION_TYPE) != null
                ? dcaAssessmentPerfModel.getString(AJEntityDailyClassActivity.COLLECTION_TYPE)
                : null);
        contentBody.put(JsonConstants.ASSESSMENT, assessmentData);


        List<Map> assessmentQuestionsKPI = Base.findAll(
            AJEntityDailyClassActivity.SELECT_ASSESSMENT_QUESTION_FOREACH_COLLID_AND_SESSION_ID,
            context.collectionId(), studentLatestSessionId, date,
            AJEntityDailyClassActivity.ATTR_CRP_EVENTNAME);
          ...
          for (Map questions : assessmentQuestionsKPI) {
            ...
            qnData
                .put(JsonConstants.ANSWER_OBJECT,
                    questions.get(AJEntityDailyClassActivity.ANSWER_OBJECT) != null
                        ? new JsonArray(
                            questions.get(AJEntityDailyClassActivity.ANSWER_OBJECT).toString())
                        : null);
            // Rubrics - Score should be NULL only incase of OE questions
            qnData.put(JsonConstants.RAW_SCORE,
                questions.get(AJEntityDailyClassActivity.SCORE) != null
                    ? Math.round(
                        Double.valueOf(questions.get(AJEntityDailyClassActivity.SCORE).toString()))
                    : "NA");
            Object reactionObj =
                Base.firstCell(AJEntityDailyClassActivity.SELECT_ASSESSMENT_RESOURCE_REACTION,
            ...
            qnData.put(JsonConstants.MAX_SCORE,
                questions.get(AJEntityDailyClassActivity.MAX_SCORE) != null
                    ? Math.round(Double
                        .valueOf(questions.get(AJEntityDailyClassActivity.MAX_SCORE).toString()))
                    : "NA");
            Double quesScore = questions.get(AJEntityDailyClassActivity.SCORE) != null
                ? Double.valueOf(questions.get(AJEntityDailyClassActivity.SCORE).toString())
                : null;
            Double maxScore = questions.get(AJEntityDailyClassActivity.MAX_SCORE) != null
                ? Double.valueOf(questions.get(AJEntityDailyClassActivity.MAX_SCORE).toString())
                : 0.0;
            ...
            if (qnData.getString(EventConstants.QUESTION_TYPE)
              ...
              Object isGradedObj =
                  Base.firstCell(AJEntityDailyClassActivity.GET_OE_QUE_GRADE_STATUS,
                      context.collectionId(), studentLatestSessionId,
                      questions.get(AJEntityDailyClassActivity.RESOURCE_ID).toString(), date);
