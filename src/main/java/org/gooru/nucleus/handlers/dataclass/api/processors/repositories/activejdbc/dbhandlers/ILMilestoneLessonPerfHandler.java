  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (!lessonIDforMilestone.isEmpty()) {
      ...
      if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
        lessonDataList =
            Base.findAll(AJEntityBaseReports.SELECT_IL_MILESTONE_LESSON_PERF_FOR_COLLECTION,
                context.courseId(), userId, listToPostgresArrayString(lessonIds));
      } else {
        lessonDataList = Base.findAll(
            AJEntityBaseReports.SELECT_IL_MILESTONE_LESSON_PERF_FOR_ASSESSMENT, context.courseId(),
        ...
        for (Map m : lessonDataList) {
          ...
          if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
            scoreCompletionMap = Base.findAll(
                AJEntityBaseReports.GET_IL_COLL_COMPLETED_COUNT_SCORE_FOREACH_LESSON_ID,
                context.courseId(), this.lessonId, userId, EventConstants.COLLECTION_PLAY);
            scoreMap =
                Base.findAll(AJEntityBaseReports.GET_IL_MILESTONE_COLL_SCORE_FOREACH_LESSON_ID,
                    context.courseId(), this.lessonId, userId);
          } else {
            scoreCompletionMap = Base.findAll(
                AJEntityBaseReports.GET_IL_ASMT_COMPLETED_COUNT_SCORE_FOREACH_LESSON_ID,
                context.courseId(), this.lessonId, userId, EventConstants.COLLECTION_PLAY,
                EventConstants.STOP);
          }
          ...
          scoreCompletionMap.forEach(sc -> {
            ...
            lessonData.put(AJEntityBaseReports.ATTR_SCORE,
                sc.get(AJEntityBaseReports.ATTR_SCORE) != null
                    ? Math.round(Double.valueOf(sc.get(AJEntityBaseReports.ATTR_SCORE).toString()))
                    : null);
