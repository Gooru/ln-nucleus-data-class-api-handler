 ...
 * 
 */
public class ILMilestoneLessonPerfHandler implements DBHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(ILMilestoneLessonPerfHandler.class);
    ...
    public ILMilestoneLessonPerfHandler(ProcessorContext context) {
    ...
    public ExecutionResult<MessageResponse> validateRequest() {
      if (context.getUserIdFromRequest() == null
          ...
          return new ExecutionResult<>(MessageResponseFactory.createForbiddenResponse("Invalid User"), ExecutionStatus.FAILED);
      }
      ...
      lessonIDforMilestone.forEach(lesson -> lessonIds.add(lesson.getString(AJEntityMilestoneLessonMap.LESSON_ID)));
        ...
        if (!lessonIDforMilestone.isEmpty()) {                    
          ...
          if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
            lessonDataList = Base.findAll(AJEntityBaseReports.SELECT_IL_MILESTONE_LESSON_PERF_FOR_COLLECTION, context.courseId(),
                  userId, listToPostgresArrayString(lessonIds));
          } else {
        	  lessonDataList = Base.findAll(AJEntityBaseReports.SELECT_IL_MILESTONE_LESSON_PERF_FOR_ASSESSMENT, context.courseId(),
          ...
          if (!lessonDataList.isEmpty()) {
            for (Map m : lessonDataList) {
              ...
              if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
            	  scoreCompletionMap = Base.findAll(AJEntityBaseReports.GET_IL_COLL_COMPLETED_COUNT_SCORE_FOREACH_LESSON_ID,
                      context.courseId(), this.lessonId, userId, EventConstants.COLLECTION_PLAY);
                scoreMap = Base.findAll(AJEntityBaseReports.GET_IL_MILESTONE_COLL_SCORE_FOREACH_LESSON_ID,
                        context.courseId(), this.lessonId, userId);
              }else{
            	  scoreCompletionMap = Base.findAll(AJEntityBaseReports.GET_IL_ASMT_COMPLETED_COUNT_SCORE_FOREACH_LESSON_ID,
