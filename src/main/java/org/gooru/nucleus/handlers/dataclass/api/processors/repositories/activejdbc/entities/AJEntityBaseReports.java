 ...
 // GET STUDENT's PEERS IN COURSE
  ...
  //Getting RESOURCE DATA (reaction)
   ...
   //NU Data Reports 
    ...
    public static final String DCA_MONTHLY_USAGE_ASSESSMENT_AGG_DATA = "SELECT assessmentData.month as month, max(score) as score, sum(time_spent) as time_spent FROM "
            + "(select FIRST_VALUE(score) OVER (PARTITION BY collection_id ORDER BY updated_at desc) AS score, "
            + "time_spent,to_char(updated_at,'Mon') as month from base_reports where class_id = ? AND actor_id = ? "
            + "AND event_name = 'collection.play' AND collection_type = 'assessment' "
            + "AND event_type = 'stop' AND collection_type = 'assessment' "
    ...
    public static final String DCA_MONTHLY_USAGE_ASSESSMENT_DATA = "select collection_id, max(score) as score, sum(time_spent) as time_spent from "
            ...
            + "time_spent from base_reports where class_id = ? AND actor_id = ? AND event_name = 'collection.play' AND "
            + "collection_type = 'assessment' AND event_type = 'stop' AND collection_type = 'assessment' AND "
            + "extract(year from updated_at) = ? AND to_char(updated_at,'Mon') =  ?) as assessmentData group by collection_id";
    
    public static final String DCA_WEEKLY_USAGE_ASSESSMENT_AGG_DATA = "SELECT assessmentData.week as week, max(score) as score, sum(time_spent) as time_spent FROM "
            + "(select FIRST_VALUE(score) OVER (PARTITION BY collection_id ORDER BY updated_at desc) AS score, "
            + "time_spent,extract(week from updated_at) as week from base_reports where class_id = ? AND actor_id = ? "
            + "AND event_name = 'collection.play' AND collection_type = 'assessment' "
            + "AND event_type = 'stop' AND collection_type = 'assessment' "
            + "AND extract(year from updated_at) = ? AND to_char(updated_at,'Mon') =  ?) as assessmentData group by week";
    
    public static final String DCA_WEEKLY_USAGE_ASSESSMENT_DATA = "select collection_id, max(score) as score, sum(time_spent) as time_spent from "
            + "(select collection_id, FIRST_VALUE(score) OVER (PARTITION BY collection_id ORDER BY updated_at desc) AS score, "
            + "time_spent from base_reports where class_id = ? AND actor_id = ? AND event_name = 'collection.play' AND "
            + "collection_type = 'assessment' AND event_type = 'stop' AND collection_type = 'assessment' AND "
            + "extract(year from updated_at) = ? AND to_char(updated_at,'Mon') =  ? AND extract(week from updated_at) = ?) as assessmentData group by collection_id";
    
    public static final String DCA_MONTHLY_USAGE_COLLECTION_AGG_DATA = "SELECT collectionData.month, SUM(collectionData.time_spent) as time_spent, "
            + "SUM(collectionData.score) AS score, SUM(collectionData.max_score) AS max_score "
            + "FROM (SELECT DISTINCT ON (resource_id) collection_id,resource_type,time_spent, "
            + "FIRST_VALUE(score) OVER (PARTITION BY resource_id ORDER BY updated_at desc) AS score, "
            + "FIRST_VALUE(max_score) OVER (PARTITION BY resource_id ORDER BY updated_at desc) AS max_score, "
            + "to_char(updated_at,'Mon') as month FROM base_reports WHERE class_id = ? actor_id = ? "
            + "AND event_name = 'collection.resource.play' AND extract(year from updated_at) = ?) AS collectionData GROUP BY collectionData.month";
   
    public static final String DCA_MONTHLY_USAGE_COLLECTION_DATA = "SELECT collectionData.collection_id, SUM (collectionData.time_spent) as time_spent, "
            + "SUM(collectionData.score) AS score, SUM(collectionData.max_score) AS max_score "
            + "FROM (SELECT DISTINCT ON (resource_id) collection_id,time_spent, "
            + "FIRST_VALUE(score) OVER (PARTITION BY resource_id ORDER BY updated_at desc) AS score, "
            + "FIRST_VALUE(max_score) OVER (PARTITION BY resource_id ORDER BY updated_at desc) AS max_score, "
            + "to_char(updated_at,'Mon') as month FROM base_reports "
    ...
    public static final String DCA_WEEKLY_USAGE_COLLECTION_AGG_DATA = "SELECT collectionData.week, SUM(collectionData.time_spent) as time_spent, "
            + "SUM(collectionData.score) AS score, SUM(collectionData.max_score) AS max_score "
            + "FROM (SELECT DISTINCT ON (resource_id) collection_id,resource_type,time_spent, "
            + "FIRST_VALUE(score) OVER (PARTITION BY resource_id ORDER BY updated_at desc) AS score, "
            + "FIRST_VALUE(max_score) OVER (PARTITION BY resource_id ORDER BY updated_at desc) AS max_score, "
            + "extract(week from updated_at) as week FROM base_reports WHERE class_id = ? actor_id = ? "
            + "AND event_name = 'collection.resource.play' AND extract(year from updated_at) = ? AND to_char(updated_at,'Mon') =  ?) AS collectionData GROUP BY collectionData.week";
   
    public static final String DCA_WEEKLY_USAGE_COLLECTION_DATA = "SELECT collectionData.collection_id, SUM (collectionData.time_spent) as time_spent, "
            + "SUM(collectionData.score) AS score, SUM(collectionData.max_score) AS max_score "
            + "FROM (SELECT DISTINCT ON (resource_id) collection_id,time_spent, "
            + "FIRST_VALUE(score) OVER (PARTITION BY resource_id ORDER BY updated_at desc) AS score, "
            + "FIRST_VALUE(max_score) OVER (PARTITION BY resource_id ORDER BY updated_at desc) AS max_score, "
            + "to_char(updated_at,'Mon') as month FROM base_reports "
            + "WHERE class_id = ? actor_id = ? AND event_name = 'collection.resource.play' "
            + "AND extract(year from updated_at) = ? AND to_char(updated_at,'Mon') =  ? AND extract(week from updated_at) = ?) AS collectionData "
            + "GROUP BY collectionData.collection_id";
