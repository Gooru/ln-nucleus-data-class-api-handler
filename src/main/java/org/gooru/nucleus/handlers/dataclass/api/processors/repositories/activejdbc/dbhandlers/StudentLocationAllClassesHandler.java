...
public class StudentLocationAllClassesHandler implements DBHandler {
          ...
          for (Object cls : classes) {
              ...
              if ((classObject.containsKey(EventConstants.CLASS_ID) && 
            	  ...
            	  if (!classMap.containsKey(classObject.getString(EventConstants.CLASS_ID))) {
            		  classMap.put(classObject.getString(EventConstants.CLASS_ID), 
            				  classObject);
            		  LOGGER.debug("Inserted JsonObject {}  for classId key {}", classObject, classObject.getString(EventConstants.CLASS_ID));
            	  } else if (classMap.containsKey(classObject.getString(EventConstants.CLASS_ID))){
            		  LOGGER.error("Multipe Courses associated with class {}, ignoring course {} for this class ", 
            				  classObject.getString(EventConstants.CLASS_ID), classObject.getString(EventConstants.COURSE_ID));
            	  }
                  ...
                  //This case should not arise. However if it does, since fwCode is associated with Course, & since course is not present,
            	  //fwCode is also assumed to be absent. Even if fwCode is present, in the absence of course, it should be considered as an 
    ...
    @SuppressWarnings("rawtypes")
    private void createLocationResponseForAllClasses(JsonArray locArray, List<Map> studLocData, String fwCode) {
        if (!studLocData.isEmpty()) {
            studLocData.forEach(m -> {
                ...
                String courseId = m.get(AJEntityBaseReports.COURSE_GOORU_OID) != null ? m.get(AJEntityBaseReports.COURSE_GOORU_OID).toString() : null;
                ...
                AJEntityBaseReports collectionStatus = AJEntityBaseReports.findFirst("session_id = ?  AND collection_id = ? AND event_name = ? AND event_type = ?", 
                		m.get(AJEntityBaseReports.SESSION_ID).toString(), collectionId, EventConstants.COLLECTION_PLAY, EventConstants.STOP);
                if (collectionStatus != null){
                  ...
                  if (Objects.equals(m.get(AJEntityBaseReports.COLLECTION_TYPE), EventConstants.ASSESSMENT) ||
                		  Objects.equals(m.get(AJEntityBaseReports.COLLECTION_TYPE), EventConstants.EXT_ASSESSMENT)) {
                      studLoc.put(AJEntityBaseReports.ATTR_SCORE, collectionStatus.get(AJEntityBaseReports.SCORE) == null 
