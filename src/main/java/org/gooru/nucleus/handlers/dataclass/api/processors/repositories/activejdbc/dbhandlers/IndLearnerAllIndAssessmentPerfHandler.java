...
public class IndLearnerAllIndAssessmentPerfHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(IndLearnerAllIndAssessmentPerfHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    String query = StringUtil.isNullOrEmpty(limitS)
        ? AJEntityBaseReports.GET_IL_ALL_ASSESSMENT_ATTEMPTS_TIMESPENT
        : AJEntityBaseReports.GET_IL_ALL_ASSESSMENT_ATTEMPTS_TIMESPENT + "LIMIT "
            + Long.valueOf(limitS);

    List<Map> assessmentTS = Base.findAll(query, this.userId,
        StringUtil.isNullOrEmpty(offsetS) ? 0L : Long.valueOf(offsetS));
    if (!assessmentTS.isEmpty()) {
      assessmentTS.forEach(assessmentTsKpi -> {
        ...
        assesmentObject.put(AJEntityBaseReports.ATTR_ATTEMPTS,
            Long.parseLong(assessmentTsKpi.get(AJEntityBaseReports.ATTR_ATTEMPTS).toString()));
        List<Map> assessmentCompletionKpi =
            Base.findAll(AJEntityBaseReports.GET_IL_ALL_ASSESSMENT_SCORE_COMPLETION, this.userId,
                assessmentTsKpi.get(AJEntityBaseReports.COLLECTION_OID).toString());
        if (!assessmentCompletionKpi.isEmpty()) {
          assessmentCompletionKpi.forEach(courseComplettion -> assesmentObject.put(
              AJEntityBaseReports.ATTR_SCORE,
              courseComplettion.get(AJEntityBaseReports.ATTR_SCORE) == null ? null
                  : Math.round(Double
                      .valueOf(courseComplettion.get(AJEntityBaseReports.ATTR_SCORE).toString()))));
        } else {
