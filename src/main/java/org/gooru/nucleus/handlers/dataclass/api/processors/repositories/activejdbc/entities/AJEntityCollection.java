...
public class AJEntityCollection extends Model {    
    ...
    public static AJEntityCollection fetchCollection(String collectionId) throws Throwable {
        ...
        try {
          ...
          List<AJEntityCollection> results = AJEntityCollection.findBySQL("select title from collection where id = ?::uuid and is_deleted = ?", collectionId, false);
    ...
    public static AJEntityCollection fetchCollectionByLesson(String collectionId, String lessonId) throws Throwable {
        DB coreDb = new DB("coreDb");
        try {
          coreDb.open(DataSourceRegistry.getInstance().getCoreDataSource());
          coreDb.connection().setReadOnly(true);
          coreDb.openTransaction();
          List<AJEntityCollection> results = AJEntityCollection.findBySQL("select title from collection where id = ?::uuid and lesson_id = ?::uuid and is_deleted = ?", collectionId, lessonId, false);
          coreDb.commitTransaction();
          if (results != null && !results.isEmpty()) {
            return results.get(0);
          }
