...
public class StudentPeersInUnitHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    List<Map> UnitPeerMap = Base.findAll(AJEntityBaseReports.GET_PEERS_COUNT_IN_UNIT, classId,
    ...
    if (!UnitPeerMap.isEmpty()) {
      ...
      UnitPeerMap.forEach(m -> {
        ...
        StudentPeerArray.add(new JsonObject()
            .put(AJEntityBaseReports.ATTR_LESSON_ID,
      ...
      LOGGER.error(
          "Student Peers Count for Unit cannot be obtained. There may be no peers for this Unit");
    }

    resultBody.put(JsonConstants.CONTENT, StudentPeerArray).putNull(JsonConstants.MESSAGE)
