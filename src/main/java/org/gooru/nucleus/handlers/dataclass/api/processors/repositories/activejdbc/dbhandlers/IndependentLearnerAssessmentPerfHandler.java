...
public class IndependentLearnerAssessmentPerfHandler implements DBHandler {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(IndependentLearnerAssessmentPerfHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    for (String userID : userIds) {
      ...
      studentLatestAttempt = Base.findAll(
          AJEntityBaseReports.GET_INDEPENDENT_LEARNER_LATEST_COMPLETED_SESSION_ID,
          context.courseId(), context.unitId(), context.lessonId(), context.collectionId(), userID);
      if (!studentLatestAttempt.isEmpty()) {
        studentLatestAttempt.forEach(attempts -> {
          List<Map> assessmentQuestionsKPI = Base.findAll(
              ...
              context.collectionId(), attempts.get(AJEntityBaseReports.SESSION_ID).toString(),
              AJEntityBaseReports.ATTR_CRP_EVENTNAME);
          JsonArray questionsArray = new JsonArray();
          if (!assessmentQuestionsKPI.isEmpty()) {
            assessmentQuestionsKPI.forEach(questions -> {
              JsonObject qnData = ValueMapper.map(
                  ResponseAttributeIdentifier.getSessionAssessmentQuestionAttributesMap(),
                  questions);
