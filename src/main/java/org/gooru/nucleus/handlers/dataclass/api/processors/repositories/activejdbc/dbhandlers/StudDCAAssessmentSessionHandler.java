...
public class StudDCAAssessmentSessionHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(StudDCAAssessmentSessionHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (StringUtil.isNullOrEmpty(eDate) || StringUtil.isNullOrEmpty(sDate)) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "Date Missing. Cannot fetch Assessment Sessions in Daily Class Activity"),
    ...
    if (StringUtil.isNullOrEmpty(classId)) {
      ...
      return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
          "ClassId Missing. Cannot fetch Session Information"), ExecutionStatus.FAILED);
    }
    ...
    distinctSessionsList =
        Base.findAll(AJEntityDailyClassActivity.GET_ASMT_USER_SESSIONS_FOR_COLLID, classId,
      ...
      distinctSessionsList.forEach(m -> {
        ...
        if (!sessionStatusMap.isEmpty()) {
          ...
          sessionStatusMap.forEach(sess -> {
            if (Objects.equals(sess.get(AJEntityDailyClassActivity.EVENTTYPE).toString(),
                EventConstants.START)) {
              sessionObj
                  .put(JsonConstants.EVENT_TIME,
                      sess.get(AJEntityDailyClassActivity.UPDATE_TIMESTAMP).toString())
