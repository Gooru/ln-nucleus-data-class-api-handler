...
public class StudPerfMultipleCollectionHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(StudPerfMultipleCollectionHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (collectionIds.isEmpty()) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "CollectionIds are Missing. Cannot fetch Student Performance for Assessments"),
          ExecutionStatus.FAILED);
    }
    ...
    if (StringUtil.isNullOrEmpty(userId)) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "User Id Missing. Cannot fetch Student Performance in Assessment"),
          ExecutionStatus.FAILED);
    ...
    if (!StringUtil.isNullOrEmpty(classId)) {
      ...
      collectionPerf = Base.findAll(AJEntityBaseReports.GET_PERFORMANCE_FOR_CLASS_COLLECTION,
          classId, listToPostgresArrayString(collIds), userId);
    } else {
    ...
    /**
     * for (String collId : collIds) { List<Map> collTSA = null; LOGGER.debug("The collectionIds
     * are" + collId); JsonObject collectionKpi = new JsonObject();
     * 
     * //Find Timespent and Attempts collTSA =
     * Base.findAll(AJEntityBaseReports.GET_TOTAL_TIMESPENT_ATTEMPTS_FOR_COLLECTION, collId,
     * AJEntityBaseReports.ATTR_COLLECTION, this.userId, EventConstants.COLLECTION_PLAY);
     * 
     * if (!collTSA.isEmpty()) { collTSA.forEach(m -> {
     * collectionKpi.put(AJEntityBaseReports.ATTR_TIMESPENT,
     * m.get(AJEntityBaseReports.ATTR_TIMESPENT).toString());
