...
public class IndependentLearnerLocationHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(IndependentLearnerLocationHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (!StringUtil.isNullOrEmpty(contentType)
      ...
      LazyList<AJEntityILBookmarkContent> ILAssessments = AJEntityILBookmarkContent
          .findBySQL(AJEntityILBookmarkContent.SELECT_DISTINCT_IL_ASSESSMENT_ID, userId);
      if (!ILAssessments.isEmpty()) {
        ILAssessments.forEach(
            a -> assessmentIds.add(a.get(AJEntityILBookmarkContent.CONTENT_ID).toString()));
        ILlocMap = Base.findAll(AJEntityBaseReports.GET_DISTINCT_ASSESSMENT_FOR_INDEPENDENT_LEARNER,
            listToPostgresArrayString(assessmentIds), userId,
            (StringUtil.isNullOrEmpty(limitS) || (Integer.valueOf(limitS) > MAX_LIMIT)) ? MAX_LIMIT
                : Integer.valueOf(limitS),
            StringUtil.isNullOrEmpty(offsetS) ? 0 : Integer.valueOf(offsetS));
      }
      ...
      LazyList<AJEntityILBookmarkContent> ILCollections = AJEntityILBookmarkContent
          .findBySQL(AJEntityILBookmarkContent.SELECT_DISTINCT_IL_COLLECTION_ID, userId);
      if (!ILCollections.isEmpty()) {
        ILCollections.forEach(
            a -> collectionIds.add(a.get(AJEntityILBookmarkContent.CONTENT_ID).toString()));
        ILlocMap = Base.findAll(AJEntityBaseReports.GET_DISTINCT_COLLECTION_FOR_INDEPENDENT_LEARNER,
            listToPostgresArrayString(collectionIds), userId,
            (StringUtil.isNullOrEmpty(limitS) || (Integer.valueOf(limitS) > MAX_LIMIT)) ? MAX_LIMIT
                : Integer.valueOf(limitS),
            StringUtil.isNullOrEmpty(offsetS) ? 0 : Integer.valueOf(offsetS));
      }
    ...
    if ((ILlocMap != null) && (!ILlocMap.isEmpty())) {
      ILlocMap.forEach(m -> {
        ...
        if (contentType.equalsIgnoreCase(MessageConstants.COURSE)) {
          String coId = m.get(AJEntityBaseReports.COURSE_GOORU_OID) != null
          ...
          ILloc.put(JsonConstants.COLLECTION_TITLE,
          ...
          List<Map> completedCourseCollection =
              Base.findAll(AJEntityBaseReports.GET_IL_COURSE_COLLECTION_STATUS, coId, sessionId,
                  collectionId, EventConstants.COLLECTION_PLAY, EventConstants.STOP);
          if (!completedCourseCollection.isEmpty()) {
            ...
            if (collectionType.equalsIgnoreCase(MessageConstants.ASSESSMENT)
              ...
              ILloc.put(AJEntityBaseReports.ATTR_SCORE,
                  score.get(AJEntityBaseReports.ATTR_SCORE) == null ? null
                      : Math.round(
                          Double.valueOf(score.get(AJEntityBaseReports.ATTR_SCORE).toString())));
            }
          } else {
          ...
          if (fwCode != null && coId != null && unitId != null && lessonId != null) {
            ...
            AJEntityMilestone milestone = AJEntityMilestone.findFirst(
                ...
                UUID.fromString(coId), UUID.fromString(unitId), UUID.fromString(lessonId), fwCode);
            if (milestone != null) {
