...
public class IndLearnerCollectionSummaryHandler implements DBHandler {
    ...
    if (context.getUserIdFromRequest() == null) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createForbiddenResponse(
              "UserID is mandatory to fetch Independent Learner Collection Performance"),
          ExecutionStatus.FAILED);
    }
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (!StringUtil.isNullOrEmpty(courseId) && !StringUtil.isNullOrEmpty(unitId)
      ...
      collectionMaximumScore = Base.findAll(AJEntityBaseReports.SELECT_IL_COLLECTION_MAX_SCORE,
          courseId, unitId, lessonId, collectionId, this.userId);
    } else {
      collectionMaximumScore = Base.findAll(
          AJEntityBaseReports.SELECT_IL_STANDALONE_COLLECTION_MAX_SCORE, collectionId, this.userId);
    }
    ...
    if (!StringUtil.isNullOrEmpty(courseId) && !StringUtil.isNullOrEmpty(unitId)
        && !StringUtil.isNullOrEmpty(lessonId)) {
      lastAccessedTime =
          Base.findAll(AJEntityBaseReports.SELECT_COURSE_IL_COLLECTION_LAST_ACCESSED_TIME, courseId,
              unitId, lessonId, collectionId, this.userId);
    } else {
      lastAccessedTime =
          Base.findAll(AJEntityBaseReports.SELECT_IL_STANDALONE_COLLECTION_LAST_ACCESSED_TIME,
    ...
    if (!StringUtil.isNullOrEmpty(courseId) && !StringUtil.isNullOrEmpty(unitId)
        && !StringUtil.isNullOrEmpty(lessonId)) {
      collectionData = Base.findAll(AJEntityBaseReports.SELECT_IL_COLLECTION_AGG_DATA, courseId,
          unitId, lessonId, collectionId, this.userId);
    } else {
      // Currently, data for Collections at the IL Landing page should be inclusive of CUL and
      // Standalone collections
      collectionData = Base.findAll(AJEntityBaseReports.SELECT_IL_STANDALONE_COLLECTION_AGG_DATA,
          collectionId, this.userId);
    }
    if (!collectionData.isEmpty()) {
      ...
      collectionData.forEach(m -> {
        ...
        if (!StringUtil.isNullOrEmpty(courseId) && !StringUtil.isNullOrEmpty(unitId)
            && !StringUtil.isNullOrEmpty(lessonId)) {
          collectionScore = Base.firstCell(AJEntityBaseReports.SELECT_IL_COLLECTION_AGG_SCORE,
              courseId, unitId, lessonId, collectionId, this.userId);
        } else {
          collectionScore =
              Base.firstCell(AJEntityBaseReports.SELECT_IL_STANDALONE_COLLECTION_AGG_SCORE,
                  collectionId, this.userId);
        }
        ...
        if (!StringUtil.isNullOrEmpty(courseId) && !StringUtil.isNullOrEmpty(unitId)
            && !StringUtil.isNullOrEmpty(lessonId)) {
          collectionReaction = Base.firstCell(AJEntityBaseReports.SELECT_IL_COLLECTION_AGG_REACTION,
              courseId, unitId, lessonId, collectionId, this.userId);
        } else {
          collectionReaction =
              Base.firstCell(AJEntityBaseReports.SELECT_IL_STANDALONE_COLLECTION_AGG_REACTION,
                  collectionId, this.userId);
        }
      ...
      if (!StringUtil.isNullOrEmpty(courseId) && !StringUtil.isNullOrEmpty(unitId)
        ...
        assessmentQuestionsKPI =
            Base.findAll(AJEntityBaseReports.SELECT_IL_COLLECTION_RESOURCE_AGG_DATA, courseId,
                unitId, lessonId, collectionId, this.userId);
      } else {
        assessmentQuestionsKPI =
            Base.findAll(AJEntityBaseReports.SELECT_IL_STANDALONE_COLLECTION_RESOURCE_AGG_DATA,
                collectionId, this.userId);
      }
      ...
      if (!assessmentQuestionsKPI.isEmpty()) {
        assessmentQuestionsKPI.forEach(questions -> {
          ...
          if (!StringUtil.isNullOrEmpty(courseId) && !StringUtil.isNullOrEmpty(unitId)
              && !StringUtil.isNullOrEmpty(lessonId)) {
            questionScore =
                Base.findAll(AJEntityBaseReports.SELECT_IL_COLLECTION_QUESTION_AGG_SCORE, courseId,
                    unitId, lessonId, collectionId, questions.get(AJEntityBaseReports.RESOURCE_ID),
                    this.userId);
          } else {
            questionScore =
                Base.findAll(AJEntityBaseReports.SELECT_IL_STANDALONE_COLLECTION_QUESTION_AGG_SCORE,
          ...
          if (!StringUtil.isNullOrEmpty(courseId) && !StringUtil.isNullOrEmpty(unitId)
            ...
            if (latestSessionId != null && qnData.getString(EventConstants.QUESTION_TYPE)
              ...
              Object isGradedObj = Base.firstCell(AJEntityBaseReports.SELECT_IL_OE_QUE_GRADE_STATUS,
          ...
          if (!StringUtil.isNullOrEmpty(courseId) && !StringUtil.isNullOrEmpty(unitId)
              && !StringUtil.isNullOrEmpty(lessonId)) {
            resourceReaction =
                Base.findAll(AJEntityBaseReports.SELECT_IL_COLLECTION_RESOURCE_AGG_REACTION,
                    courseId, unitId, lessonId, collectionId,
                    questions.get(AJEntityBaseReports.RESOURCE_ID), this.userId);
          } else {
            resourceReaction = Base.findAll(
                AJEntityBaseReports.SELECT_IL_STANDALONE_COLLECTION_RESOURCE_AGG_REACTION,
                collectionId, questions.get(AJEntityBaseReports.RESOURCE_ID), this.userId);
          }
