...
public class IndependentLearnerCourseAllItemsPerformanceHandler implements DBHandler {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(IndependentLearnerCourseAllItemsPerformanceHandler.class);
    ...
    public ExecutionResult<MessageResponse> executeRequest() {
        ...
        for (String userID : userIds) {
            ...
            List<Map> assessmentKpi = Base.findAll(AJEntityCollectionPerformance.SELECT_IL_ITEM_PERF_IN_CLASS, courseId, userID, startDate, offset, limit);
            if (!assessmentKpi.isEmpty()) {
                ...
                assessmentKpi.forEach(m -> {
                    ...
                    if (collectionType.equalsIgnoreCase(JsonConstants.COLLECTION)) {
                        ...
                        collectionKpi.put(AJEntityBaseReports.ATTR_ATTEMPTS, m.get(AJEntityBaseReports.ATTR_ATTEMPTS) != null ? Integer.valueOf(m.get(AJEntityBaseReports.ATTR_ATTEMPTS).toString()) : null);
                    }

                    collectionKpi.put(AJEntityBaseReports.ATTR_COLLECTION_TYPE, m.get(AJEntityBaseReports.ATTR_COLLECTION_TYPE).toString());
    ...
    private List<String> fetchUserId() {
        ...
        if (StringUtil.isNullOrEmpty(userId)) {
            LOGGER.warn("UserID is not in the request to fetch Student Performance of all items in Course. Assume user is a teacher");
            LazyList<AJEntityCollectionPerformance> userIdOfClass = AJEntityCollectionPerformance.findBySQL(AJEntityCollectionPerformance.SELECT_IL_DISTINCT_USERID_FOR_COURSE_ID, this.courseId);
