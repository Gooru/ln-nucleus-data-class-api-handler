...
public class IndLearnerCourseAssessmentsPerfHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(IndLearnerCourseAssessmentsPerfHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (!StringUtil.isNullOrEmpty(startDate) && !FieldValidator.validateDate(startDate)) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "Invalid startDate. Cannot fetch Student Performance in Assessment"),
          ExecutionStatus.FAILED);
    ...
    if (!StringUtil.isNullOrEmpty(endDate) && !FieldValidator.validateDate(endDate)) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "Invalid endDate. Cannot fetch Student Performance in Assessment"),
          ExecutionStatus.FAILED);
    ...
    for (String collId : collIds) {
      ...
      StringBuilder assessTSAQuery = new StringBuilder(
          AJEntityBaseReports.GET_IL_COURSE_ASSESSMENTS_TOTAL_TIME_SPENT_ATTEMPTS);
      ...
      if (!StringUtil.isNullOrEmpty(startDate)) {
        assessTSAQuery.append(AJEntityBaseReports.SPACE).append(AJEntityBaseReports.AND)
            .append(AJEntityBaseReports.SPACE)
            .append(AJEntityBaseReports.UPDATDED_AT_GREATER_THAN_OR_EQUAL);
      ...
      assessTSA =
          AJEntityBaseReports.findBySQL(assessTSAQuery.toString(), assessTSAParams.toArray());

      if (!assessTSA.isEmpty()) {
        assessTSA.forEach(m -> {
          assessmentKpi.put(AJEntityBaseReports.ATTR_TIME_SPENT,
              Long.parseLong(m.get(AJEntityBaseReports.TIME_SPENT).toString()));
          assessmentKpi.put(AJEntityBaseReports.ATTR_ATTEMPTS,
              Integer.parseInt(m.get(AJEntityBaseReports.VIEWS).toString()));
        });
      }

      StringBuilder assessScoreQuery =
          new StringBuilder(AJEntityBaseReports.GET_IL_COURSE_ASSESSMENTS_SCORE);

      if (!StringUtil.isNullOrEmpty(startDate)) {
        assessScoreQuery.append(AJEntityBaseReports.SPACE).append(AJEntityBaseReports.AND)
            .append(AJEntityBaseReports.SPACE)
            .append(AJEntityBaseReports.UPDATDED_AT_GREATER_THAN_OR_EQUAL);
      }
      if (!StringUtil.isNullOrEmpty(endDate)) {
      ...
      assessScore =
          AJEntityBaseReports.findBySQL(assessScoreQuery.toString(), assessTSAParams.toArray());
      String latestSessionId = null;

      if (!assessScore.isEmpty()) {
        latestSessionId =
            ((AJEntityBaseReports) assessScore.get(0)).getString(AJEntityBaseReports.SESSION_ID);
        assessScore.forEach(m -> {
          assessmentKpi.put(AJEntityBaseReports.ATTR_SCORE,
