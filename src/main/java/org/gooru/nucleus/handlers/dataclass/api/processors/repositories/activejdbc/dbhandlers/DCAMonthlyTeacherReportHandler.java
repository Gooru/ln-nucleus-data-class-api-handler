...
public class DCAMonthlyTeacherReportHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> checkSanity() {
    if (context.request() == null || context.request().isEmpty()) {
      ...
      return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse("Invalid data provided to fetch DCA weekly/monhly report"),
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    ...
    if (context.request().getString(REQUEST_DIMENSION) == null) {
      return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse("dimension should be provided"), ExecutionStatus.FAILED);
      ...
      return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse("Valid Dimensions are : weekly,monthly"),
              ExecutionStatus.FAILED);
    }

    if (context.request().getString(REQUEST_COLLECTION_TYPE) == null) {
      return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse("collectionType should be provided!"), ExecutionStatus.FAILED);
    }
    return new ExecutionResult<>(null, ExecutionStatus.CONTINUE_PROCESSING);
  }
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    year = context.request().getString(REQUEST_YEAR) != null ? Integer.parseInt(context.request().getString(REQUEST_YEAR)) : 0;
