...
public class AJEntityCourseCollectionCount extends Model {
  ...
  public static final String GET_MILESTONE_LESSON_ASSESSMENT_COUNT = "SELECT (SUM(assessment_count) + SUM(ext_assessment_count)) as totalCount FROM course_collection_count WHERE "
	  		+ "course_id = ? AND lesson_id = ?";
  
  public static final String GET_MILESTONE_ASSESSMENT_COUNT = "SELECT (SUM(assessment_count) + SUM(ext_assessment_count)) as totalCount FROM course_collection_count WHERE "
	  		+ "course_id = ? AND lesson_id = ANY(?::varchar[])";
  
  public static final String GET_MILESTONE_LESSON_COLLECTION_COUNT = "SELECT (SUM(collection_count) + SUM(ext_collection_count)) as totalCount FROM course_collection_count WHERE "
	  		+ "course_id = ? AND lesson_id = ?";

public static final String GET_MILESTONE_COLLECTION_COUNT = "SELECT (SUM(collection_count) + SUM(ext_collection_count)) as totalCount FROM course_collection_count WHERE "
	  		+ "course_id = ? AND lesson_id = ANY(?::varchar[])";
