...
public class IndLearnerAllIndCollectionLocHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(IndLearnerAllIndCollectionLocHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (StringUtil.isNullOrEmpty(userId)) {
      ...
      return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
          "User Id Missing. Cannot fetch Learner's Collections"), ExecutionStatus.FAILED);
    ...
    ILAssessments =
        Base.findAll(AJEntityBaseReports.GET_DISTINCT_COLLECTION_FOR_INDEPENDENT_LEARNER, userId);
    ...
    if (!ILAssessments.isEmpty()) {
      ILAssessments.forEach(m -> {
        ...
        if (!Base.findAll(AJEntityBaseReports.GET_IL_COLLECTION_STATUS, sessionId, collectionId,
            EventConstants.COLLECTION_PLAY, EventConstants.STOP).isEmpty()) {
