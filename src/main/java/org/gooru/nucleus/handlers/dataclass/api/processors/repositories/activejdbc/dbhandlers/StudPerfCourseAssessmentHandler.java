...
public class StudPerfCourseAssessmentHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(StudPerfCourseAssessmentHandler.class);
    ...
    if (StringUtil.isNullOrEmpty(userId1)) {
      ...
      LazyList<AJEntityBaseReports> userIdOfClass =
          AJEntityBaseReports.findBySQL(AJEntityBaseReports.SELECT_DISTINCT_USERID_FOR_COURSE_ID
              + AJEntityBaseReports.ADD_ASS_TYPE_FILTER_TO_QUERY, classId, courseId);
    ...
    for (String userID : userIds) {
      ...
      for (String collId : collIds) {
        ...
        assessTSA = Base.findAll(AJEntityBaseReports.GET_TOTAL_TIME_SPENT_ATTEMPTS_FOR_ASSESSMENT,
            classId, courseId, collId, AJEntityBaseReports.ATTR_CP_EVENTNAME, userID,
            EventConstants.STOP);
          ...
          assessTSA.forEach(m -> {
            ...
            assessmentKpi.put(AJEntityBaseReports.ATTR_ATTEMPTS,
                m.get(AJEntityBaseReports.ATTR_ATTEMPTS) != null
                    ? Integer.parseInt(m.get(AJEntityBaseReports.ATTR_ATTEMPTS).toString())
                    : null);
          });
        }

        // Get the latest Score
        assessScore = Base.findAll(AJEntityBaseReports.GET_LATEST_SCORE_FOR_ASSESSMENT, classId,
            courseId, collId, userID, EventConstants.COLLECTION_PLAY, EventConstants.STOP);
        String latestSessionId = null;

        if (!assessScore.isEmpty()) {
          latestSessionId =
              (String) assessScore.get(0).getOrDefault(AJEntityBaseReports.SESSION_ID, null);
