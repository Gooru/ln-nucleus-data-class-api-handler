...
public class StudentUnitPerfHandler implements DBHandler {
    ...
    if (context.request() == null || context.request().isEmpty()) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "Invalid data provided to fetch Student Performance in Units"),
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (StringUtil.isNullOrEmpty(collectionType)) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "CollectionType Missing. Cannot fetch Student Performance in Units"),
          ExecutionStatus.FAILED);
    }
    ...
    if (StringUtil.isNullOrEmpty(userId)) {
      LOGGER.warn(
          "UserID is not in the request to fetch Student Performance in Units. Assume user is a teacher");
      LazyList<AJEntityBaseReports> userIdforUnit = AJEntityBaseReports.findBySQL(
          AJEntityBaseReports.SELECT_DISTINCT_USERID_FOR_UNIT_ID + addCollTypeFilterToQuery,
          context.classId(), context.courseId(), context.unitId());
    ...
    for (String userID : userIds) {
      ...
      lessonIDforUnit = AJEntityBaseReports.findBySQL(
          AJEntityBaseReports.SELECT_DISTINCT_LESSON_ID_FOR_UNIT_ID + addCollTypeFilterToQuery,
          context.classId(), context.courseId(), context.unitId(), userID);
      if (!lessonIDforUnit.isEmpty()) {
        LOGGER.debug("Got a list of Distinct lessonIDs for this Unit");

        lessonIDforUnit.forEach(
            lesson -> lessonIds.add(lesson.getString(AJEntityBaseReports.LESSON_GOORU_OID)));
        List<Map> lessonKpi;
        if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
          lessonKpi = Base.findAll(AJEntityBaseReports.SELECT_STUDENT_UNIT_PERF_FOR_COLLECTION,
              context.classId(), context.courseId(), context.unitId(), userID,
              listToPostgresArrayString(lessonIds));
        } else {
          lessonKpi = Base.findAll(AJEntityBaseReports.SELECT_STUDENT_UNIT_PERF_FOR_ASSESSMENT,
              context.classId(), context.courseId(), context.unitId(), userID,
        ...
        if (!lessonKpi.isEmpty()) {
          for (Map m : lessonKpi) {
            ...
            if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
              completedCountMap =
                  Base.findAll(AJEntityBaseReports.GET_COMPLETED_COLL_COUNT_FOREACH_LESSON_ID,
                      context.classId(), context.courseId(), context.unitId(), this.lessonId,
                      userID, EventConstants.COLLECTION_PLAY);
              scoreMap = Base.findAll(AJEntityBaseReports.GET_COLL_SCORE_FOREACH_LESSON_ID,
                  context.classId(), context.courseId(), context.unitId(), this.lessonId, userID);
            } else {
              completedCountMap =
                  Base.findAll(AJEntityBaseReports.GET_COMPLETED_ASMT_COUNT_FOREACH_LESSON_ID,
                      context.classId(), context.courseId(), context.unitId(), this.lessonId,
                      userID, EventConstants.COLLECTION_PLAY);
            }
            JsonObject lessonData =
                ValueMapper.map(ResponseAttributeIdentifier.getUnitPerformanceAttributesMap(), m);
            completedCountMap.forEach(scoreCompletonMap -> {
              lessonData.put(AJEntityBaseReports.ATTR_COMPLETED_COUNT, Integer.valueOf(
            ...
            if (lessonAssessmentCountMap.containsKey(this.lessonId)) {
              ...
              Object classTotalCount =
                  Base.firstCell(AJEntityCourseCollectionCount.GET_LESSON_ASSESSMENT_COUNT,
            ...
            LazyList<AJEntityBaseReports> collIDforlesson = AJEntityBaseReports.findBySQL(
                AJEntityBaseReports.SELECT_DISTINCT_COLLID_FOR_LESSON_ID_WO_PATH_ID
            ...
            if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
              assessmentKpi = Base.findAll(
                  AJEntityBaseReports.SELECT_STUDENT_LESSON_PERF_FOR_COLLECTION_WO_PATH_ID,
                  context.classId(), context.courseId(), context.unitId(), this.lessonId,
                  listToPostgresArrayString(collIds), userID);
            } else {
              assessmentKpi = Base.findAll(
                  AJEntityBaseReports.SELECT_STUDENT_LESSON_PERF_FOR_ASSESSMENT_WO_PATH_ID,
                  context.classId(), context.courseId(), context.unitId(), this.lessonId,
                  listToPostgresArrayString(collIds), userID, EventConstants.COLLECTION_PLAY);
            }
            if (!assessmentKpi.isEmpty()) {
              assessmentKpi.forEach(ass -> {
                JsonObject assData = ValueMapper
                    .map(ResponseAttributeIdentifier.getLessonPerformanceAttributesMap(), ass);
                assData.put(AJEntityBaseReports.ATTR_COMPLETED_COUNT, 1);
                // Since this is the leaf-level data, TOTAL_COUNT doesn't make sense here. It will
                // be stuffed as 1
                // for compatibility. (should not be used in any report calculations)
                assData.put(AJEntityBaseReports.ATTR_TOTAL_COUNT, 1);
                assData.put(AJEntityBaseReports.ATTR_SCORE,
                    ass.get(AJEntityBaseReports.ATTR_SCORE) != null
                        ? Math.round(
                            Double.valueOf(ass.get(AJEntityBaseReports.ATTR_SCORE).toString()))
                        : null);
                ...
                if (this.collectionType.equalsIgnoreCase(JsonConstants.COLLECTION)) {
                  ...
                  collectionQuestionCount = Base.findAll(
                      AJEntityBaseReports.SELECT_COLLECTION_SCORE_AND_MAX_SCORE, context.classId(),
                ...
                String latestSessionId = ass.get(AJEntityBaseReports.SESSION_ID) != null
                    ? ass.get(AJEntityBaseReports.SESSION_ID).toString()
                    : null;
