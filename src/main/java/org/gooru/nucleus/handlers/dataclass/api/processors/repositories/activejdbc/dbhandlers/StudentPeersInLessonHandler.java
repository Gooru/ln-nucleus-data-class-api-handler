  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (StringUtil.isNullOrEmpty(userId1)) {
      ...
      return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(
          "UserId Missing. Cannot fetch student list"), ExecutionStatus.FAILED);

    }

    // If CollectionType is Assessment
    // @NU Resource as Suggestions - include event_name = 'collection.play' to ensure peers at C/A
    // level
    LazyList<AJEntityBaseReports> collIDforAssessment =
        AJEntityBaseReports.findBySQL(AJEntityBaseReports.GET_PEERS_IN_LESSON, context.classId(),
            userId1, context.courseId(), context.unitId(), context.lessonId());
    ...
    if (!collIDforAssessment.isEmpty()) {
      collIDforAssessment.forEach(coll -> {
        ...
        String collTypeParam =
            collectionType.equalsIgnoreCase(JsonConstants.COLLECTION) ? JsonConstants.COLLECTIONID
                : JsonConstants.ASSESSMENTID;
        ...
        if (((currTs.getTime() - userTs.getTime()) < timeDiff)) {
          peerArray.add(new JsonObject().put(collTypeParam, assessmentId)
              .put(JsonConstants.USERUID, userId).put(JsonConstants.STATUS, JsonConstants.ACTIVE));
        } else if (((currTs.getTime() - userTs.getTime()) > timeDiff)) {
          peerArray.add(
              new JsonObject().put(collTypeParam, assessmentId).put(JsonConstants.USERUID, userId)
      ...
      LOGGER.error(
          "Student Peers Count for Lessons cannot be obtained. There may be no peers for this Lessons");
    }

    resultBody.put(JsonConstants.CONTENT, peerArray).putNull(JsonConstants.MESSAGE)
