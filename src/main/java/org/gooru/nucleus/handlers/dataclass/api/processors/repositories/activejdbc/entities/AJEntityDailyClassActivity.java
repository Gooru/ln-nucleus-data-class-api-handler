...
//            + "WHERE class_id = ? AND collection_id = ? AND collection_type =? AND date_in_time_zone BETWEEN ? AND ?";
  ...
  //*************************************************************************************************************************
    ...
    //************************************************************************************************************************
    public static final String DCA_CLASS_SCORE_MONTHLY_SUMMARY = "SELECT monthAsInt, month, AVG(score) AS score FROM (SELECT month, monthAsInt, collection_id, AVG(score) AS score FROM (SELECT actor_id, EXTRACT(MONTH FROM date_in_time_zone) AS monthAsInt, to_char(date_in_time_zone,'Mon') AS month, collection_id, AVG(score) as score FROM daily_class_activity WHERE class_id = ? AND event_name = 'collection.play' AND collection_type IN ('assessment','assessment-external') AND event_type = 'stop' AND extract(year from date_in_time_zone) = ? group by actor_id, collection_id, date_in_time_zone) ca group by collection_id, month, monthAsInt) c group by month, monthAsInt";
    
    public static final String DCA_CLASS_TS_SUMMARY_FOR_MONTH = "SELECT ROUND(AVG(time_spent)) AS time_spent FROM (SELECT collection_id, round(avg(time_spent)) AS time_spent FROM (SELECT actor_id, collection_id, SUM (time_spent) as time_spent FROM (SELECT DISTINCT ON (date_in_time_zone, actor_id, resource_id) actor_id, collection_id, FIRST_VALUE(time_spent) OVER (PARTITION BY date_in_time_zone, actor_id, resource_id ORDER BY updated_at desc) as time_spent FROM daily_class_activity WHERE class_id = ? AND event_name = 'collection.resource.play' AND collection_type IN ('collection', 'collection-external') AND extract(year from date_in_time_zone) = ? AND extract(month from date_in_time_zone) =  ?) AS collectionData GROUP BY actor_id, collection_id) ca group by collection_id) c";
    ...
    public static final String DCA_CLASS_COLL_SUMMARY_FOR_MONTH = "SELECT collection_id, collection_type, ROUND(AVG (time_spent)) AS time_spent, AVG(score) AS score, AVG(max_score) AS max_score FROM (SELECT actor_id, collection_id, collection_type, SUM(time_spent) as time_spent, SUM(score) AS score, SUM(collectionData.max_score) AS max_score FROM (SELECT DISTINCT ON (date_in_time_zone,actor_id, resource_id) actor_id, collection_id, collection_type, FIRST_VALUE(time_spent) OVER (PARTITION BY date_in_time_zone, actor_id, resource_id ORDER BY updated_at desc) as time_spent, FIRST_VALUE(score) OVER (PARTITION BY date_in_time_zone, actor_id, resource_id ORDER BY updated_at desc) AS score, FIRST_VALUE(max_score) OVER (PARTITION BY date_in_time_zone, actor_id, resource_id ORDER BY updated_at desc) AS max_score FROM daily_class_activity WHERE class_id = ? AND event_name = 'collection.resource.play' AND collection_type IN ('collection', 'collection-external') AND extract(year from date_in_time_zone) = ? AND extract(month from date_in_time_zone) =  ?) AS collectionData GROUP BY actor_id, collection_id, collection_type) a GROUP by collection_id, collection_type";
    ...
    public static final String DCA_CLASS_USER_USAGE_COLLECTION_DATA = "SELECT actor_id, ROUND(AVG(time_spent)) AS time_spent, AVG(score) AS score, AVG(max_score) AS max_score FROM (SELECT actor_id, collection_id, SUM (time_spent) as time_spent, SUM(score) AS score, SUM(collectionData.max_score) AS max_score FROM (SELECT DISTINCT ON (date_in_time_zone,actor_id, resource_id) collection_id, actor_id,FIRST_VALUE(time_spent) OVER (PARTITION BY date_in_time_zone, actor_id, resource_id ORDER BY updated_at desc) AS time_spent, FIRST_VALUE(score) OVER (PARTITION BY date_in_time_zone, actor_id, resource_id ORDER BY updated_at desc) AS score, FIRST_VALUE(max_score) OVER (PARTITION BY date_in_time_zone, actor_id, resource_id ORDER BY updated_at desc) AS max_score FROM daily_class_activity WHERE class_id = ? AND collection_id = ? AND event_name = 'collection.resource.play' AND extract(year from date_in_time_zone) = ? AND extract(month from date_in_time_zone) =  ?) AS collectionData GROUP BY actor_id, collection_id) a GROUP by actor_id";
    //*************************************************************************************************************************
