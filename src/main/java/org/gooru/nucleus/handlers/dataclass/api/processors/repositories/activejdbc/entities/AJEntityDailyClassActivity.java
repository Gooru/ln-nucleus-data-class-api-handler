...
//            + "WHERE class_id = ? AND collection_id = ? AND collection_type =? AND date_in_time_zone BETWEEN ? AND ?";
  ...
  //*************************************************************************************************************************
    ...
    public static final String DCA_CLASS_COLL_SUMMARY_FOR_MONTH = "SELECT collection_id, collection_type, ROUND(AVG (time_spent)) AS time_spent FROM (SELECT actor_id, collection_id, collection_type, SUM(time_spent) as time_spent FROM (SELECT actor_id, collection_id, collection_type, time_spent FROM daily_class_activity WHERE class_id = ? AND event_name = 'collection.resource.play' AND collection_type IN ('collection', 'collection-external') AND extract(year from date_in_time_zone) = ? AND extract(month from date_in_time_zone) =  ?) AS collectionData GROUP BY actor_id, collection_id, collection_type) a GROUP by collection_id, collection_type";
