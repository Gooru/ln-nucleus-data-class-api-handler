...
public class DataReportsHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    LazyList<AJEntityBaseReports> distinctCourses = AJEntityBaseReports.findBySQL(
        AJEntityBaseReports.NU_DISTINCT_COURSES, context.getUserIdFromRequest(), context.classId(),
    ...
    distinctCourses.forEach(courses -> {
      ...
      LazyList<AJEntityBaseReports> distinctUnits = AJEntityBaseReports.findBySQL(
          AJEntityBaseReports.NU_DISTINCT_UNITS, context.getUserIdFromRequest(), context.classId(),
          courseId, context.startDate(), context.endDate());
      distinctUnits.forEach(units -> {
        ...
        LazyList<AJEntityBaseReports> distinctLessons = AJEntityBaseReports.findBySQL(
            AJEntityBaseReports.NU_DISTINCT_LESSONS, context.getUserIdFromRequest(),
            context.classId(), courseId, unitId, context.startDate(), context.endDate());
        ...
        distinctLessons.forEach(lesson -> {
          ...
          LazyList<AJEntityBaseReports> distinctCollections =
              AJEntityBaseReports.findBySQL(AJEntityBaseReports.NU_DISTINCT_COLLECTIONS,
                  context.getUserIdFromRequest(), context.classId(), courseId, unitId, lessonId,
                  context.startDate(), context.endDate());
          ...
          distinctCollections.forEach(collections -> {
            ...
            if (collections.getString(AJEntityBaseReports.COLLECTION_TYPE)
                .equalsIgnoreCase(JsonConstants.COLLECTION)) {
              List<Map> collectionQuestionCount =
                  Base.findAll(AJEntityBaseReports.NU_SELECT_COLLECTION_QUESTION_COUNT,
                      context.classId(), courseId, unitId, lessonId, collectionId,
                      context.getUserIdFromRequest(), context.startDate(), context.endDate());
              ...
              collectionQuestionCount.forEach(qc -> {
                if (qc.get(AJEntityBaseReports.QUESTION_COUNT) != null) {
                  questionCount =
                      Integer.valueOf(qc.get(AJEntityBaseReports.QUESTION_COUNT).toString());
                }
              });

              List<Map> collectionData =
                  Base.findAll(AJEntityBaseReports.NU_SELECT_COLLECTION_AGG_DATA, context.classId(),
                      courseId, unitId, lessonId, collectionId, context.getUserIdFromRequest(),
                      context.startDate(), context.endDate());

              LOGGER.debug("Collection Attributes obtained");
              collectionData.forEach(m -> {
                ValueMapper.map(collectionObject,
                    ResponseAttributeIdentifier.getSessionCollectionAttributesMap(), m);
                collectionObject.put(EventConstants.COLLECTION_TYPE,
                    AJEntityBaseReports.ATTR_COLLECTION);
                collectionObject.put(JsonConstants.SCORE,
                ...
                if (questionCount > 0) {
                  ...
                  collectionScore =
                      Base.firstCell(AJEntityBaseReports.NU_SELECT_COLLECTION_AGG_SCORE,
                          context.classId(), courseId, unitId, lessonId, collectionId,
                          context.getUserIdFromRequest(), context.startDate(), context.endDate());

                  if (collectionScore != null) {
                    scoreInPercent =
                        ((Double.valueOf(collectionScore.toString()) / questionCount) * 100);
                  }
                }
                LOGGER.debug("Collection score : {} - collectionId : {}",
                    Math.round(scoreInPercent), collectionId);
                collectionObject.put(AJEntityBaseReports.SCORE, Math.round(scoreInPercent));
                Object collectionReaction;
                collectionReaction =
                    Base.firstCell(AJEntityBaseReports.NU_SELECT_COLLECTION_AGG_REACTION,
                        context.classId(), courseId, unitId, lessonId, collectionId,
                        context.getUserIdFromRequest(), context.startDate(), context.endDate());
                if (collectionReaction != null) {
                  reaction = Integer.valueOf(collectionReaction.toString());
                }
                LOGGER.debug("Collection reaction : {} - collectionId : {}", reaction,
                    collectionId);
              ...
              List<Map> assessmentQuestionsKPI =
                  Base.findAll(AJEntityBaseReports.NU_SELECT_COLLECTION_RESOURCE_AGG_DATA,
                      context.classId(), courseId, unitId, lessonId, collectionId,
                      context.getUserIdFromRequest(), context.startDate(), context.endDate());
              ...
              if (!assessmentQuestionsKPI.isEmpty()) {
                assessmentQuestionsKPI.forEach(questions -> {
                  JsonObject qnData = ValueMapper.map(
                      ResponseAttributeIdentifier.getSessionCollectionResourceAttributesMap(),
                      questions);
                  qnData.put(AJEntityBaseReports.ATTR_RESOURCE_ID,
                  ...
                  qnData.put("metadata", getTitleAndTaxonomy(
                  ...
                  qnData.put(JsonConstants.SCORE, Math
                      .round(Double.valueOf(questions.get(AJEntityBaseReports.SCORE).toString())));
                  if (questionCount > 0) {
                    List<Map> questionScore =
                        Base.findAll(AJEntityBaseReports.NU_SELECT_COLLECTION_QUESTION_AGG_SCORE,
                            context.classId(), courseId, unitId, lessonId, collectionId,
                            questions.get(AJEntityBaseReports.RESOURCE_ID),
                            context.getUserIdFromRequest(), context.startDate(), context.endDate());
                    ...
                    if (!questionScore.isEmpty()) {
                      questionScore.forEach(qs -> {
                        qnData.put(JsonConstants.SCORE, Math.round(
                            Double.valueOf(qs.get(AJEntityBaseReports.SCORE).toString()) * 100));
                        qnData.put(JsonConstants.ANSWER_OBJECT,
                        ...
                        LOGGER.debug("Question Score : {} - resourceId : {}",
                            qs.get(AJEntityBaseReports.SCORE).toString(),
                            questions.get(AJEntityBaseReports.RESOURCE_ID));
                      });
                    }
                  }
                  List<Map> resourceReaction =
                      Base.findAll(AJEntityBaseReports.NU_SELECT_COLLECTION_RESOURCE_AGG_REACTION,
                          context.classId(), courseId, unitId, lessonId, collectionId,
                          questions.get(AJEntityBaseReports.RESOURCE_ID),
                          context.getUserIdFromRequest(), context.startDate(), context.endDate());
              ...
              LazyList<AJEntityBaseReports> assesmentScoreReaction = AJEntityBaseReports.findBySQL(
                  AJEntityBaseReports.GET_NU_REPORT_ASSESSMENT_SCORE_REACTION,
                  context.getUserIdFromRequest(), context.classId(), courseId, unitId, lessonId,
                  collectionId, context.startDate(), context.endDate());
              assesmentScoreReaction.forEach(scoreReaction -> {
                collectionObject.put(AJEntityBaseReports.ATTR_SCORE, Math.round(
                    Double.valueOf(scoreReaction.get(AJEntityBaseReports.SCORE).toString())));
                collectionObject.put(AJEntityBaseReports.ATTR_REACTION, Math.round(
                    Double.valueOf(scoreReaction.get(AJEntityBaseReports.REACTION).toString())));

              });
              collectionObject.put(AJEntityBaseReports.ATTR_COLLECTION_TYPE,
                  AJEntityBaseReports.ATTR_ASSESSMENT);
              LazyList<AJEntityBaseReports> assesmentAttemptsTs = AJEntityBaseReports.findBySQL(
                  AJEntityBaseReports.GET_NU_REPORT_ASSESSMENT_TS_ATTEMPTS,
                  context.getUserIdFromRequest(), context.classId(), courseId, unitId, lessonId,
                  collectionId, context.startDate(), context.endDate());

              assesmentAttemptsTs.forEach(attemptsTs -> {
                collectionObject.put(AJEntityBaseReports.ATTR_TIME_SPENT,
                    attemptsTs.getLong(AJEntityBaseReports.TIME_SPENT));
                collectionObject.put(AJEntityBaseReports.ATTR_ATTEMPTS,
                    attemptsTs.getLong(AJEntityBaseReports.VIEWS));

              });

              LazyList<AJEntityBaseReports> qnScoreReaction = AJEntityBaseReports.findBySQL(
                  AJEntityBaseReports.GET_NU_REPORT_ASSESSMENT_QUESTIONS_SCORE_REACTION,
                  context.getUserIdFromRequest(), context.classId(), courseId, unitId, lessonId,
                  collectionId, context.startDate(), context.endDate());
              JsonArray questionsArray = new JsonArray();
              qnScoreReaction.forEach(scoreReaction -> {
                ...
                qnData.put("metadata", getTitleAndTaxonomy(
                    scoreReaction.get(AJEntityBaseReports.RESOURCE_ID).toString()));


                qnData.put(AJEntityBaseReports.ATTR_QUESTION_ID,
                    scoreReaction.get(AJEntityBaseReports.RESOURCE_ID));
                qnData.put(JsonConstants.RESOURCE_TYPE,
                    scoreReaction.get(AJEntityBaseReports.RESOURCE_TYPE));
                qnData.put(JsonConstants.QUESTION_TYPE,
                    scoreReaction.get(AJEntityBaseReports.QUESTION_TYPE));
                qnData.put(JsonConstants.REACTION, scoreReaction.get(AJEntityBaseReports.REACTION));
                qnData
                    .put(JsonConstants.ANSWER_OBJECT,
                        scoreReaction.get(AJEntityBaseReports.ANSWER_OBECT) != null
                            ? new JsonArray(
                                scoreReaction.get(AJEntityBaseReports.ANSWER_OBECT).toString())
                            : null);
                ...
                qnData.put(JsonConstants.SCORE,
                    scoreReaction.get(AJEntityBaseReports.SCORE) != null
                        ? Math.round(
                            Double.valueOf(scoreReaction.get(AJEntityBaseReports.SCORE).toString()))
                        : "NA");
                LazyList<AJEntityBaseReports> qnTSAttempts = AJEntityBaseReports.findBySQL(
                    AJEntityBaseReports.GET_NU_REPORT_ASSESSMENT_QUESTIONS_TS_ATTEMPTS,
                    context.getUserIdFromRequest(), context.classId(), courseId, unitId, lessonId,
                    collectionId, scoreReaction.get(AJEntityBaseReports.RESOURCE_ID).toString(),
                    context.startDate(), context.endDate());
                qnTSAttempts.forEach(tsAttempts -> {
                  qnData.put(AJEntityBaseReports.ATTR_TIME_SPENT,
                      tsAttempts.get(AJEntityBaseReports.TIME_SPENT));
                  qnData.put(AJEntityBaseReports.ATTR_ATTEMPTS,
                      tsAttempts.get(AJEntityBaseReports.VIEWS));
                });
                questionsArray.add(qnData);
              });
  ...
  private JsonObject getTitleAndTaxonomy(String id) {
    ...
    meta.forEach(me -> {
      ...
      metaData.put(AJEntityContent.TAXONOMY,
          me.get(AJEntityContent.TAXONOMY) != null
              ? new JsonObject(me.get(AJEntityContent.TAXONOMY).toString())
              : null);
    });
    return metaData;
  }
