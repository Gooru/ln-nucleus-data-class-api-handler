...
public class StudCACollectionSessionPerfHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(StudCACollectionSessionPerfHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    List<Map> collectionMaximumScore =
        Base.findAll(AJEntityDailyClassActivity.SELECT_COLLECTION_MAX_SCORE_FOR_SESSION,
    ...
    List<Map> lastAccessedTime =
        Base.findAll(AJEntityDailyClassActivity.SELECT_LAST_ACCESSED_TIME_OF_SESSION,
            context.sessionId(), userId);
    ...
    List<Map> collectionData =
        Base.findAll(AJEntityDailyClassActivity.SELECT_COLLECTION_AGG_DATA_FOR_SESSION,
            context.sessionId(), userId);
    ...
    if (!collectionData.isEmpty()) {
      ...
      collectionData.forEach(m -> {
        ...
        Object collectionScore =
            Base.firstCell(AJEntityDailyClassActivity.SELECT_COLLECTION_AGG_SCORE_FOR_SESSION,
                context.sessionId(), userId);
        ...
        Object collectionReaction =
            Base.firstCell(AJEntityDailyClassActivity.SELECT_COLLECTION_AGG_REACTION_FOR_SESSION,
                context.sessionId(), userId);
      ...
      collectionQuestionsKPI =
          Base.findAll(AJEntityDailyClassActivity.SELECT_COLLECTION_RESOURCE_AGG_DATA_FOR_SESSION,
              context.sessionId(), userId);
      ...
      if (!collectionQuestionsKPI.isEmpty()) {
        collectionQuestionsKPI.forEach(questions -> {
          ...
          List<Map> questionScore = Base.findAll(
              AJEntityDailyClassActivity.SELECT_COLLECTION_QUESTION_AGG_SCORE_FOR_SESSION,
              context.sessionId(), userId, questions.get(AJEntityDailyClassActivity.RESOURCE_ID));
          ...
          if (qnData.getString(EventConstants.QUESTION_TYPE)
            ...
            Object isGradedObj = Base.firstCell(
                AJEntityDailyClassActivity.GET_OE_QUE_GRADE_STATUS_FOR_SESSION_ID,
                context.sessionId(), userId, questions.get(AJEntityDailyClassActivity.RESOURCE_ID));
          ...
          resourceReaction = Base.findAll(
              AJEntityDailyClassActivity.SELECT_COLLECTION_RESOURCE_AGG_REACTION_FOR_SESSION,
              context.sessionId(), userId, questions.get(AJEntityDailyClassActivity.RESOURCE_ID));
