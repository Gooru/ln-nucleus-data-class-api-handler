...
public class ILMilestonePerfHandler implements DBHandler {
	...
	public ExecutionResult<MessageResponse> executeRequest() {
		...
		milestoneIDforCourse = AJEntityMilestoneLessonMap.findBySQL(
				AJEntityMilestoneLessonMap.SELECT_DISTINCT_MILESTONE_ID_FOR_COURSE, UUID.fromString(context.courseId()),
				fwCode);

		if (!milestoneIDforCourse.isEmpty()) {
			milestoneIDforCourse.forEach(
					milestone -> milestoneIds.add(milestone.getString(AJEntityMilestoneLessonMap.MILESTONE_ID)));
		}
		...
		for (String mileId : milestoneIds) {
			...
			if (!lessonIDforMilestone.isEmpty()) {
				...
				if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
					contentPerf = Base.findAll(AJEntityBaseReports.SELECT_IL_STUDENT_MILESTONE_PERF_FOR_COLLECTION,
							context.courseId(), userId, listToPostgresArrayString(lessonIds));
				} else {
					contentPerf = Base.findAll(AJEntityBaseReports.SELECT_IL_STUDENT_MILESTONE_PERF_FOR_ASSESSMENT,
							context.courseId(), userId, listToPostgresArrayString(lessonIds),
						...
						scoreCompletionMap.forEach(sc -> {
							...
							mileData.put(AJEntityBaseReports.ATTR_SCORE,
									sc.get(AJEntityBaseReports.ATTR_SCORE) != null
											? Math.round(
													Double.valueOf(sc.get(AJEntityBaseReports.ATTR_SCORE).toString()))
											: null);
						});
						...
						if (this.collectionType.equalsIgnoreCase(EventConstants.COLLECTION)) {
							if (milestoneCollectionCountMap.containsKey(mileId)) {
								...
								Object classTotalCount = Base.firstCell(
										AJEntityCourseCollectionCount.GET_MILESTONE_COLLECTION_COUNT, context.courseId(),
							...
							if (milestoneAssessmentCountMap.containsKey(mileId)) {
								...
								Object classTotalCount = Base.firstCell(
										AJEntityCourseCollectionCount.GET_MILESTONE_ASSESSMENT_COUNT, context.courseId(),
										listToPostgresArrayString(lessonIds));
								totalCount = classTotalCount != null ? (Integer.valueOf(classTotalCount.toString())) : 0;
				...
				contentBody.put(JsonConstants.USAGE_DATA, milestonePerfArray).put(JsonConstants.USERUID, userId)
						.put(JsonConstants.MILESTONE_ID, mileId);
