...
import org.gooru.nucleus.handlers.dataclass.api.app.components.RedisClient;
import org.gooru.nucleus.handlers.dataclass.api.constants.HttpConstants;
...
public class AuthVerticle extends AbstractVerticle {
  ...
  public void start(Future<Void> voidFuture) throws Exception {
    ...
    eb.consumer(MessagebusEndpoints.MBEP_AUTH, message -> {
      LOG.debug("Received message in body: " + message.body());
      LOG.debug("Received message in header: " + message.headers());
  ...
  private static JsonObject getAccessToken(String token) {
    ...
    if (token != null) {
      try {
        accessTokenInfo = RedisClient.getInstance().getJsonObject(token);
        if (accessTokenInfo != null) {
          int expireAtInSeconds = accessTokenInfo.getInteger(ACCESS_TOKEN_VALIDITY);
          RedisClient.getInstance().expire(token, expireAtInSeconds);
        } else {
