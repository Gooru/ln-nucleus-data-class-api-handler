    ...
    if (!StringUtil.isNullOrEmpty(userId)) {
      for (Map<String, String> classMap : classList) {
        ...
        if (!StringUtil.isNullOrEmpty(courseId)) {
          List<Map> classPerfData = Base.findAll(AJEntityBaseReports.SELECT_STUDENT_ALL_CLASS_DATA,
            ...
            classPerfData.forEach(classData -> {
              ...
              classKPI.put(AJEntityBaseReports.ATTR_TIME_SPENT,
                  classData.get(AJEntityBaseReports.ATTR_TIME_SPENT) == null ? 0
                      : Long
                          .valueOf(classData.get(AJEntityBaseReports.ATTR_TIME_SPENT).toString()));
              Object classTotalCount = Base
                  .firstCell(AJEntityCourseCollectionCount.GET_COURSE_ASSESSMENT_COUNT, courseId);
              classKPI.put(AJEntityBaseReports.ATTR_TOTAL_COUNT,
                  classTotalCount != null ? Integer.valueOf(classTotalCount.toString()) : 0);
              List<Map> classScoreCompletion =
                  Base.findAll(AJEntityBaseReports.SELECT_STUDENT_ALL_CLASS_COMPLETION_SCORE, clId,
                      courseId, userId);
              ...
              if (classScoreCompletion != null && !classScoreCompletion.isEmpty()) {
                classScoreCompletion.forEach(scoreKPI -> {
                  classKPI.put(AJEntityBaseReports.ATTR_COMPLETED_COUNT,
                      scoreKPI.get(AJEntityBaseReports.ATTR_COMPLETED_COUNT) == null ? 0
                          : Integer.valueOf(
                              scoreKPI.get(AJEntityBaseReports.ATTR_COMPLETED_COUNT).toString()));
                  classKPI.put(AJEntityBaseReports.ATTR_SCORE,
                      scoreKPI.get(AJEntityBaseReports.ATTR_SCORE) == null ? null
                          : Math.round(Double
                              .valueOf(scoreKPI.get(AJEntityBaseReports.ATTR_SCORE).toString())));
                });
              } else {
      ...
      for (Map<String, String> classMap : classList) {
        ...
        if (!StringUtil.isNullOrEmpty(courseId)) {
          ...
          if (!classPerfData.isEmpty()) {
            ...
            LazyList<AJEntityBaseReports> studClass = AJEntityBaseReports
                .findBySQL(AJEntityBaseReports.GET_DISTINCT_USERS_IN_CLASS, clId);
              ...
              classPerfList.forEach(scoData -> {
                ...
                classKPI.put(AJEntityBaseReports.ATTR_SCORE,
                    scoData.get(AJEntityBaseReports.ATTR_SCORE) == null ? null
                        : Math.round(Double
                            .valueOf(scoData.get(AJEntityBaseReports.ATTR_SCORE).toString())));
              });
            } else {
              classKPI.put(AJEntityBaseReports.ATTR_COMPLETED_COUNT, 0);
              classKPI.putNull(AJEntityBaseReports.ATTR_SCORE);
