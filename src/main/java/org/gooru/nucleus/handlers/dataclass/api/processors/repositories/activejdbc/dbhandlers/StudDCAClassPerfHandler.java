...
public class StudDCAClassPerfHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (!StringUtil.isNullOrEmpty(userId)) {
      ...
      List<Map> classScoreCompletion = Base.findAll(
          AJEntityDailyClassActivity.SELECT_STUDENT_CLASS_COMPLETION_SCORE, classId, userId);
      ...
      if (classScoreCompletion != null && !classScoreCompletion.isEmpty()) {
        classScoreCompletion.forEach(scoreKPI -> {
          classKPI.put(AJEntityDailyClassActivity.ATTR_COMPLETED_COUNT,
              scoreKPI.get(AJEntityDailyClassActivity.ATTR_COMPLETED_COUNT) == null ? 0
                  : Integer.valueOf(
                      scoreKPI.get(AJEntityDailyClassActivity.ATTR_COMPLETED_COUNT).toString()));
          classKPI.put(AJEntityDailyClassActivity.ATTR_SCORE,
              scoreKPI.get(AJEntityDailyClassActivity.ATTR_SCORE) == null ? null
                  : Math.round(Double
                      .valueOf(scoreKPI.get(AJEntityDailyClassActivity.ATTR_SCORE).toString())));
        });
      } else {
      ...
      LazyList<AJEntityDailyClassActivity> studClass = AJEntityDailyClassActivity
          .findBySQL(AJEntityDailyClassActivity.GET_DISTINCT_USERS_IN_CLASS, classId);
      studClass
          .forEach(users -> userList.add(users.getString(AJEntityDailyClassActivity.GOORUUID)));

      List<Map> classPerfList =
          Base.findAll(AJEntityDailyClassActivity.SELECT_ALL_STUDENT_CLASS_COMPLETION_SCORE,
        ...
        classPerfList.forEach(scoData -> {
          ...
          classKPI.put(AJEntityDailyClassActivity.ATTR_SCORE,
              scoData.get(AJEntityDailyClassActivity.ATTR_SCORE) == null ? null
                  : Math.round(Double
                      .valueOf(scoData.get(AJEntityDailyClassActivity.ATTR_SCORE).toString())));
        });
      } else {
        classKPI.put(AJEntityDailyClassActivity.ATTR_COMPLETED_COUNT, 0);
        classKPI.putNull(AJEntityDailyClassActivity.ATTR_SCORE);
