...
public class StudDCAAssessmentSummaryHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(StudDCAAssessmentSummaryHandler.class);
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    if (context.getUserIdFromRequest() == null
      ...
      Object classID = Base.firstCell(AJEntityDailyClassActivity.SELECT_CLASS_BY_SESSION_ID,
          context.collectionId(), sessionId);

      if (classID == null) {
        LOGGER.error("validateRequest() FAILED, No Class Association found.");
        return new ExecutionResult<>(
            MessageResponseFactory.createForbiddenResponse(
                "No Class Associated. DCA data can't be fetched by teacher/collaborator"),
            ExecutionStatus.FAILED);
      } else {
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (!StringUtil.isNullOrEmpty(sessionId)) {
      List<Map> assessmentKPI =
          Base.findAll(AJEntityDailyClassActivity.SELECT_ASSESSMENT_FOREACH_COLLID_AND_SESSION_ID,
              context.collectionId(), sessionId, userId, assessmentDate,
              AJEntityDailyClassActivity.ATTR_CP_EVENTNAME);
      Object assessmentReactionObject =
          Base.firstCell(AJEntityDailyClassActivity.SELECT_ASSESSMENT_REACTION_AND_SESSION_ID,
      ...
      if (!assessmentKPI.isEmpty()) {
        ...
        assessmentKPI.forEach(m -> {
          JsonObject assessmentData = ValueMapper
              .map(ResponseAttributeIdentifier.getSessionDCAAssessmentAttributesMap(), m);
          ...
          assessmentData.put(JsonConstants.REACTION,
              assessmentReactionObject != null ? ((Number) assessmentReactionObject).intValue()
                  : 0);
        ...
        List<Map> assessmentQuestionsKPI = Base.findAll(
            AJEntityDailyClassActivity.SELECT_ASSESSMENT_QUESTION_FOREACH_COLLID_AND_SESSION_ID_FOR_SUMMARY,
            context.collectionId(), sessionId, AJEntityDailyClassActivity.ATTR_CRP_EVENTNAME);

        JsonArray questionsArray = new JsonArray();
        if (!assessmentQuestionsKPI.isEmpty()) {
          assessmentQuestionsKPI.forEach(questions -> {
            JsonObject qnData = ValueMapper.map(
                ResponseAttributeIdentifier.getSessionDCAAssessmentQuestionAttributesMap(),
                questions);
            Object reactionObj =
            ...
            if (qnData.getString(EventConstants.QUESTION_TYPE)
              ...
              Object isGradedObj = Base.firstCell(
                  AJEntityDailyClassActivity.GET_ASMT_OE_QUE_GRADE_STATUS, context.collectionId(),
