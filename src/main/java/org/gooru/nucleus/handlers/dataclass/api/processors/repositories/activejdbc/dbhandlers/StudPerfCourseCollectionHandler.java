...
public class StudPerfCourseCollectionHandler implements DBHandler {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(StudPerfCourseCollectionHandler.class);
  ...
  public ExecutionResult<MessageResponse> executeRequest() {
    ...
    if (StringUtil.isNullOrEmpty(classId)) {
      ...
      return new ExecutionResult<>(
          MessageResponseFactory.createInvalidRequestResponse(
              "Class Id Missing. Cannot fetch Student Performance in Collection"),
          ExecutionStatus.FAILED);
    ...
    query.append(AJEntityBaseReports.AND).append(AJEntityBaseReports.SPACE)
        .append(AJEntityBaseReports.ACTOR_ID_IS);
    List<String> userIds;

    if (StringUtil.isNullOrEmpty(userId1)) {
      LOGGER.warn(
          "UserID is not in the request to fetch Student Performance in Course. Asseume user is a teacher");
      LazyList<AJEntityBaseReports> userIdOfClass =
          AJEntityBaseReports.findBySQL(AJEntityBaseReports.SELECT_DISTINCT_USERID_FOR_COURSE_ID
              + AJEntityBaseReports.ADD_COLL_TYPE_FILTER_TO_QUERY, classId, courseId);
    ...
    for (String userID : userIds) {
      ...
      for (String collId : collIds) {
        ...
        collTSA = Base.findAll(AJEntityBaseReports.GET_PERFORMANCE_FOR_COLLECTION, classId,
        ...
        if (!collTSA.isEmpty()) {
          collTSA.forEach(m -> {
            ...
            collectionKpi.put(AJEntityBaseReports.VIEWS,
                m.get(AJEntityBaseReports.VIEWS) != null
        ...
        collectionMaximumScore = Base.findAll(AJEntityBaseReports.GET_COLLECTION_MAX_SCORE, classId,
            courseId, collId, userID);
        ...
        if (this.maxScore > 0) {
          List<Map> collectionScoreList = Base.findAll(AJEntityBaseReports.GET_COLLECTION_SCORE,
              classId, courseId, collId, userID);
          if (collectionScoreList != null && !collectionScoreList.isEmpty()) {
            ...
            if (collectionMap.get(AJEntityBaseReports.SESSION_ID) != null)
              latestSessionId = collectionMap.get(AJEntityBaseReports.SESSION_ID).toString();
          } else {
